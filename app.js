...
(function(){
  ...
  function pAddDeadline(d){var id=d.id||("u:"+Date.now().toString(36)+Math.random().toString(36).slice(2,5));pMutate(function(o){o.deadlines.push({id:id,date:d.date,label:d.label,icon:d.icon||"🎯"});});return id;}
  ...
  function loadFoodDB(){
    try{
      ...
      fetch("base_aliments.json",{cache:"no-store"}).then(function(r){return r&&r.ok?r.json():null;}).then(function(j){
        ...
        FOOD_DB=arr;
        try{var h=document.getElementById("dayLog");if(h&&h.innerHTML)buildDayForm(h,dayDate);}catch(e){}
      }).catch(function(){});
    }catch(e){}
  }
...
function fqTokens(s){var STOP={de:1,du:1,des:1,au:1,aux:1,a:1,la:1,le:1,les:1,l:1,et:1,en:1,d:1,un:1,une:1};s=fqKey(s).replace(/[^a-z0-9]+/g," ");return s.split(" ").filter(function(t){return t.length>=2&&!STOP[t]&&!/^\d+$/.test(t);});}
  ...
  /* Codes de séance PROPRES au bloc (4, 5 ou 6 par semaine) ; CODES = repli historique. */
  function codesOf(b){var p=PROGRAM_BLOCKS[b];return (p&&p.codes&&p.codes.length)?p.codes:CODES;}
  function blockTotal(b){return PROGRAM_BLOCKS[b].weeks*codesOf(b).length;}
  function blockDone(b){var n=0,wk=PROGRAM_BLOCKS[b].weeks,cd=codesOf(b);for(var w=1;w<=wk;w++)for(var i=0;i<cd.length;i++)if(sess(b,w,cd[i]).done)n++;return n;}
  function daysUntil(iso){return Math.round((new Date(iso+"T00:00:00")-new Date(todayStr()+"T00:00:00"))/86400000);}
  /* Échéance : J-n avant, « Jour J », J+n après. Un objectif échu n'est plus figé à J-0. */
  function dlTxt(iso){if(!iso)return "";var n=daysUntil(iso);return n>0?("J-"+n):(n===0?"Jour J":("J+"+(-n)));}
  function dlPast(iso){return !!iso&&daysUntil(iso)<0;}
  ...
  /* Récupération des séances enregistrées pendant la collision de clés (format "b3|7") :
     une seule entrée valait pour TOUTE la semaine — on la réplique sur chaque code du bloc,
     ce qui restitue exactement ce qui était affiché. One-shot, marqué dans state.config. */
  function migrateWeekKeys(){
    if(!state.config)state.config={};
    if(state.config.weekKeyFix)return;
    var moved=0;
    Object.keys(state.sessions||{}).forEach(function(k){
      var m=/^(b\d+)\|(\d+)$/.exec(k);if(!m)return;
      var b=m[1],w=parseInt(m[2],10),rec=state.sessions[k];
      if(PROGRAM_BLOCKS[b])codesOf(b).forEach(function(c){
        var nk=sessKey(b,w,c);
        if(!state.sessions[nk]||!state.sessions[nk].done)state.sessions[nk]=JSON.parse(JSON.stringify(rec));
      });
      delete state.sessions[k];moved++;
    });
    state.config.weekKeyFix=1;if(moved)save();
  }
  ...
  function nextSession(){for(var bi=0;bi<BLOCK_ORDER.length;bi++){var b=BLOCK_ORDER[bi];var wk=PROGRAM_BLOCKS[b].weeks,cd=codesOf(b);for(var w=1;w<=wk;w++){for(var i=0;i<cd.length;i++){if(!sess(b,w,cd[i]).done)return{block:b,w:w,c:cd[i]};}}}return null;}
  function doneCount(){var n=0;BLOCK_ORDER.forEach(function(b){n+=blockDone(b);});return n;}
  function totalSessions(){var n=0;BLOCK_ORDER.forEach(function(b){n+=blockTotal(b);});return n;}
  ...
  function restTick(){var disp=document.querySelectorAll(".rest-disp");if(!disp.length){clearInterval(restInt);restInt=null;return;}var rem=Math.max(0,Math.round((restEnd-Date.now())/1000)),t=fmtTime(rem);disp.forEach(function(d){d.textContent=t;});if(rem<=0){clearInterval(restInt);restInt=null;disp.forEach(function(d){d.classList.add("done");});if(navigator.vibrate){try{navigator.vibrate([200,100,200]);}catch(e){}}}}
  function startRest(sec){restEnd=Date.now()+sec*1000;document.querySelectorAll(".rest-disp").forEach(function(d){d.classList.remove("done");});if(restInt)clearInterval(restInt);restTick();restInt=setInterval(restTick,250);}
  function stopRest(){if(restInt){clearInterval(restInt);restInt=null;}document.querySelectorAll(".rest-disp").forEach(function(d){d.textContent="0:00";d.classList.remove("done");});}
  ...
  function wireBnd(){
    ...
    document.addEventListener("click",function(e){
      ...
      var grp=BND_GROUP[k];
      if(grp&&bndOpen[k]){Object.keys(BND_GROUP).forEach(function(k2){
        if(k2!==k&&BND_GROUP[k2]===grp&&bndOpen[k2]){bndOpen[k2]=false;
          var h2=document.querySelector('[data-bnd="'+k2+'"]');if(h2)h2.classList.remove("open");
          var b2=document.querySelector('[data-bndb="'+k2+'"]');if(b2)b2.classList.add("collapsed");}
      });}
  ...
  function addObj(o){cfgObjs().push(o);save();}
  function delObj(id){var a=cfgObjs(),rm=null;for(var i=0;i<a.length;i++)if(a[i].id===id){rm=a[i];a.splice(i,1);break;}if(rm)(rm.sports||[]).forEach(function(sid){if(!cfgObjs().some(function(o){return (o.sports||[]).indexOf(sid)>=0;}))setSportU(sid,{on:false});});save();}
  ...
  function objAddSport(id,sid){var a=cfgObjs(),dl="";for(var i=0;i<a.length;i++)if(a[i].id===id){a[i].sports=a[i].sports||[];if(a[i].sports.indexOf(sid)<0)a[i].sports.push(sid);dl=a[i].deadline||"";break;}setSportU(sid,{on:true,deadline:dl});save();}
  function objRemoveSport(id,sid){var a=cfgObjs();for(var i=0;i<a.length;i++)if(a[i].id===id){a[i].sports=(a[i].sports||[]).filter(function(x){return x!==sid;});break;}if(!cfgObjs().some(function(o){return (o.sports||[]).indexOf(sid)>=0;}))setSportU(sid,{on:false});save();}
  /* Modifie l'échéance d'un objectif : propage à o.deadline, à chaque sport, et à l'échéance partagée (dlId). */
  function objSetDeadline(id,date){var a=cfgObjs();date=(date||"").trim();for(var i=0;i<a.length;i++){if(a[i].id===id){var o=a[i];o.deadline=date;(o.sports||[]).forEach(function(s){setSportU(s,{deadline:date});});if(date){if(o.dlId)pUpdateDeadline(o.dlId,{date:date});else o.dlId=pAddDeadline({date:date,label:o.name,icon:((o.kind||"sport")==="sport"?"🏅":(o.icon||"🎯"))});}else if(o.dlId){pRemoveDeadline(o.dlId);o.dlId=null;}break;}}save();}
  /* Rafraîchit les vues de l'onglet Objectif (sous le tiroir Réglages) après une édition d'objectif. */
  function refreshSportViews(){renderSportObjectives();renderSportTabs();renderProgram();renderTri();}
  ...
  function triLink(){return cfgObjs().filter(function(o){var s=o.sports||[];return s.indexOf("nage")>=0&&s.indexOf("velo")>=0&&s.indexOf("course")>=0;})[0]||null;}
  function syncTri(){var a=cfgActs().tri,has=!!triLink();if(!!a.enabled!==has){a.enabled=has;save();}}
  function migrateTriLink(){var a=cfgActs().tri;if(a&&a.enabled&&!triLink()){addObj({id:"obj-tri-"+Date.now(),name:actName("tri")||"Triathlon",icon:"🏊",sports:["nage","velo","course"],deadline:triRaceDate(),kind:"sport",tri:true});["nage","velo","course"].forEach(function(s){setSportU(s,{on:true});});}}
  /* Muscu = objectif à part entière (miroir de triLink). Un objectif « muscu » = un objectif solo sur le sport muscu.
     C'est LUI qui porte le nom + l'échéance du pane Musculation : renommer l'un renomme l'autre. */
  function muscuLink(){return cfgObjs().filter(function(o){var s=o.sports||[];return s.length===1&&s[0]==="muscu";})[0]||null;}
  function migrateMuscuLink(){var a=cfgActs().muscu;if(a&&a.enabled&&!muscuLink()){addObj({id:"obj-muscu-"+Date.now(),name:actName("muscu"),icon:"💪",sports:["muscu"],deadline:(a.deadline||""),kind:"sport",musc:true});setSportU("muscu",{on:true});}}
  /* Nettoyage unique : un sport activé (on:true) rattaché à aucun objectif est un orphelin (son objectif a été
     supprimé mais le flag on est resté) — on le désactive pour qu'il cesse d'apparaître dans l'onglet Objectif. */
  function migrateOrphanSportsOnce(){
    if(!state.config)state.config={};
    if(state.config._orphanSportsMigV1)return;
    var objs=cfgObjs(),u=sportsUser(),changed=false;
    Object.keys(u).forEach(function(id){
      if(u[id]&&u[id].on===true){
        var inObj=objs.some(function(o){return (o.sports||[]).indexOf(id)>=0;});
        if(!inObj){u[id].on=false;changed=true;}
      }
    });
    state.config._orphanSportsMigV1=true;save();
  }
  function objectives(){
    var out=[],covered={};
    if(actEnabled("tri")){out.push({id:"tri",icon:"🏊",name:actName("tri"),sports:["nage","velo","course"],deadline:triRaceDate()});covered.nage=covered.velo=covered.course=1;}
    if(actEnabled("muscu")){out.push({id:"muscu",icon:"💪",name:actName("muscu"),sports:["muscu"],deadline:((muscuLink()||{}).deadline||null)});covered.muscu=1;}
    cfgObjs().forEach(function(o){var sps=(o.sports||[]).slice();var isTL=sps.indexOf("nage")>=0&&sps.indexOf("velo")>=0&&sps.indexOf("course")>=0,isML=sps.length===1&&sps[0]==="muscu";if(isTL||isML){sps.forEach(function(sp){covered[sp]=1;});return;}out.push({id:o.id,icon:o.icon||"🎯",name:o.name,sports:sps,deadline:o.deadline||null,custom:true,kind:o.kind||"sport"});sps.forEach(function(sp){covered[sp]=1;});});
    if(typeof SPORTS_LIB!=="undefined")(SPORTS_LIB.order||[]).forEach(function(sid){
      if(covered[sid])return;var uc=sportUCfg(sid);
  ...
  var sportLvlNav={};
  var sportSessOpen=null;   /* "id|iso" de la séance sport ouverte en détail, ou null */
  var sportSessOpenIdx=0;   /* index de la séance dans le palier (titre + blocs) */
  var sportSdChrono=false;  /* minuteur affiché dans le détail (état éphémère, non persisté) */
  var sessEdit=null,sessBuf=null; /* édition de séances : {id,lvN} + buffer [{title,blocks:[[label,text]]}] */
  /* RENOMMÉ : ce sessKey(id,lvN) écrasait celui des séances muscu (même portée, même nom)
     → toutes les séances d'une semaine partageaient la clé "b|w". Ne jamais le renommer en sessKey. */
  function sportSessKey(id,lvN){return id+"|"+lvN;}
  function sportSessCustom(id,lvN){var m=state.sportSessCustom||{},v=m[sportSessKey(id,lvN)];return (v&&v.length)?v:null;}
  function setSportSessCustom(id,lvN,arr){if(!state.sportSessCustom)state.sportSessCustom={};state.sportSessCustom[sportSessKey(id,lvN)]=arr;save();}
  function clearSportSessCustom(id,lvN){if(state.sportSessCustom)delete state.sportSessCustom[sportSessKey(id,lvN)];save();}
  function sportSessions(id,lvN){return sportSessCustom(id,lvN)||sportSessionsForLevel(id,lvN);}
  function cloneSess(arr){return (arr||[]).map(function(s){return {title:s.title,blocks:(s.blocks||[]).map(function(b){return [b[0],b[1]];})};});}
  function shortFocus(f){f=(""+f).trim();var parts=f.split(/[\s,]+/);var w=parts.slice(0,3).join(" ");return (parts.length>3)?(w+"…"):f;}
  function sportSessionsForLevel(id,lvN){
    if(typeof SPORTS_LIB==="undefined")return [];
    var sp=SPORTS_LIB[id];if(!sp)return [];
    var L=(sp.levels||[])[lvN]||{},focus=L.focus||[];
    var tpl=(typeof SPORT_TPL!=="undefined"&&(SPORT_TPL[id]||SPORT_TPL._default))||{warm:"Échauffement progressif.",cool:"Retour au calme + étirements."};
    var out=[];
    focus.forEach(function(f,i){out.push({title:"Séance "+String.fromCharCode(65+i)+" · "+shortFocus(f),blocks:[["Échauffement",tpl.warm],["Bloc principal",f+" — travail spécifique, séries progressives avec récupération entre les efforts."],["Retour au calme",tpl.cool]]});});
    if(!out.length)out.push({title:"Séance",blocks:[["Échauffement",tpl.warm],["Travail",L.t||"Séance générale du palier"],["Retour au calme",tpl.cool]]});
    return out;
  }
  function sessEditorHTML(id,lvN){
    var arr=sessBuf||[];
    return '<div class="so-prog sess-ed"><div class="so-prog-h">✎ Séances du palier '+lvN+' — à ta main</div>'+
      arr.map(function(ss,si){
        return '<div class="sess-ed-item" data-si="'+si+'"><div class="sess-ed-top"><input type="text" class="sess-title" value="'+esc(ss.title||"")+'" placeholder="Titre de la séance"><button type="button" class="sess-del-s" data-si="'+si+'" aria-label="Supprimer la séance">🗑</button></div>'+
          (ss.blocks||[]).map(function(b,bi){return '<div class="sess-blk-row"><input type="text" class="sess-blk-label" value="'+esc(b[0]||"")+'" placeholder="Bloc"><textarea class="sess-blk-text" rows="2" placeholder="Contenu…">'+esc(b[1]||"")+'</textarea><button type="button" class="sess-del-b" data-si="'+si+'" data-bi="'+bi+'" aria-label="Supprimer le bloc">✕</button></div>';}).join("")+
          '<button type="button" class="sess-add-b" data-si="'+si+'">+ bloc</button></div>';
      }).join("")+
  ...
  var paneCollapse={}; /* replier une rubrique de sport (muscu/tri/<sport>) */
  ...
  function mergeSelected(){
    ...
    var isTri=sps.indexOf("nage")>=0&&sps.indexOf("velo")>=0&&sps.indexOf("course")>=0&&sps.length===3;
    var name,icon,tri=false;
    if(isTri){name="Triathlon";icon="🏊";tri=true;sps=["nage","velo","course"];}
    else{name=((typeof prompt!=="undefined"?prompt("Nom du groupe :",sps.map(function(s){return (SPORTS_LIB[s]||{}).name||s;}).join(" + ")):"")||"").trim();if(!name)return;icon="🏅";}
  ...
  function muscuApercuHTML(){
    ...
    var ml_=muscuLink(),dl=(ml_&&ml_.deadline)||"",dlNote=dl?'<div class="tp-note">Échéance '+esc(frDateShort(dl))+' · '+dlTxt(dl)+(dlPast(dl)?' — échue.':'.')+'</div>':"";
  ...
  function soCardHTML(id,obj){
      ...
      var dlSub=ddl?('<span class="pane-sub'+(dlPast(ddl)?' past':'')+'">'+dlTxt(ddl)+' · '+esc(frDateShort(ddl))+'</span>'):'';
      ...
      var week=upcoming.length?('<div class="so-next-h">Prochaines séances</div><div class="so-week">'+upcoming.map(function(iso2,i){var ti=(sess[i%Math.max(1,sess.length)]||{}).title||"Séance";return '<button type="button" class="so-check" data-sp="'+esc(id)+'" data-iso="'+iso2+'" data-sidx="'+(i%Math.max(1,sess.length))+'"><span class="so-d">'+esc(frDateShort(iso2))+'</span><span class="so-t">'+esc(ti)+'</span><span class="so-ck">à faire ›</span></button>';}).join("")+'</div>'):(days.length?'<div class="so-empty">Séances projetées toutes faites 🎉</div>':'<div class="so-empty">Choisis des jours d\'entraînement (Réglages) pour planifier tes séances.</div>');
      ...
      return '<div class="card pad so-card'+(selSports[id]?' sel':'')+(paneCollapse[id]!==false?' collapsed':'')+'" data-sport="'+esc(id)+'"><h2 class="page pane-title">'+esc(ic)+' '+esc(nm)+' <span class="so-lv">niv. '+cur+' → '+tgt+'</span>'+dlSub+'</h2>'+
        ...
        '<div class="so-dtxt">'+esc(L.d||"")+'</div>'+prog+week+'<button type="button" class="btn ghost so-log-add" data-sp="'+esc(id)+'">＋ Enregistrer une séance</button>'+(function(){if(sportSessOpen){var _o=sportSessOpen.split("|");if(_o[0]===id)return sportSessDetailHTML(id,_o.slice(1).join("|"),sportSessOpenIdx);}return "";})()+'</div>';
  }
  function isTriGroup(o){var s=o.sports||[];return s.indexOf("nage")>=0&&s.indexOf("velo")>=0&&s.indexOf("course")>=0;}

  /* ---- Journal des séances de sport : métriques par sport + détail + historique (parité muscu) ---- */
  var SPORT_METRICS={
    nage:    [{k:"dist",l:"Distance",u:"m",ph:"1500"},{k:"dur",l:"Durée",u:"min",ph:"30"},{k:"rpe",l:"Ressenti",u:"/10",ph:"6"}],
    course:  [{k:"dist",l:"Distance",u:"km",ph:"10"},{k:"dur",l:"Durée",u:"min",ph:"55"},{k:"rpe",l:"Ressenti",u:"/10",ph:"6"}],
    velo:    [{k:"dist",l:"Distance",u:"km",ph:"40"},{k:"dur",l:"Durée",u:"min",ph:"90"},{k:"rpe",l:"Ressenti",u:"/10",ph:"6"}],
    escalade:[{k:"voies",l:"Voies / blocs",u:"",ph:"8"},{k:"cota",l:"Cotation max",u:"",ph:"6b",text:true},{k:"dur",l:"Durée",u:"min",ph:"90"}],
    tennis:  [{k:"dur",l:"Durée",u:"min",ph:"60"},{k:"rpe",l:"Ressenti",u:"/10",ph:"6"}]
  };
  function sportMetrics(id){return SPORT_METRICS[id]||[{k:"dur",l:"Durée",u:"min",ph:"45"},{k:"rpe",l:"Ressenti",u:"/10",ph:"6"}];}

  /* ---- Zones FC : minutes passées par zone, stockées DANS le log de séance (rec.hr).
     Objectif du suivi : vérifier d'un coup d'œil que la Z2 domine (base aérobie). ---- */
  var HR_ZONES=[
    {k:"z1",l:"Z1",n:"Récup"},
    {k:"z2",l:"Z2",n:"Endurance"},
    {k:"z3",l:"Z3",n:"Tempo"},
    {k:"z4",l:"Z4",n:"Seuil"},
    {k:"z5",l:"Z5",n:"Max"}
  ];
  var HR_SPORTS={nage:1,velo:1,course:1};
  var HR_Z2_MIN=60;                                   /* seuil de « Z2 dominante », en % du temps */
  function hrTracked(id){return !!HR_SPORTS[id];}
  function hrTot(hr){var t=0;if(hr)HR_ZONES.forEach(function(z){t+=num(hr[z.k])||0;});return t;}
  function hrBarHTML(hr,cap){
    var tot=hrTot(hr);
    if(tot<=0)return '<div class="hrz-empty">Renseigne tes minutes par zone : la répartition s\'affiche ici.</div>';
    var segs=HR_ZONES.map(function(z){var v=num(hr[z.k])||0;if(v<=0)return "";var p=Math.round(v/tot*100);
      return '<i class="hrz-seg '+z.k+'" style="width:'+p+'%"><b>'+(p>=12?(z.l+" "+p+"%"):"")+'</b></i>';}).join("");
    var p2=Math.round((num(hr.z2)||0)/tot*100),ok=p2>=HR_Z2_MIN;
    return '<div class="hrz-bar">'+segs+'</div>'+
      '<div class="hrz-verdict '+(ok?"ok":"warn")+'">Z2 = '+p2+' % sur '+nFmt(tot)+' min — '+(ok?'la base aérobie domine ✅':'trop peu : la prochaine sortie, ralentis jusqu\'à pouvoir parler')+'</div>'+
      (cap?'<div class="hrz-cap">'+esc(cap)+'</div>':"");
  }
  function hrFormHTML(hr){
    var tot=hrTot(hr);
    var rows=HR_ZONES.map(function(z){var v=(hr&&hr[z.k]!=null)?hr[z.k]:"";
      return '<label class="hrz-f"><span class="hrz-k '+z.k+'">'+z.l+'</span><input type="number" inputmode="numeric" step="1" min="0" class="so-sd-z" data-z="'+z.k+'" value="'+esc(String(v))+'" placeholder="0"><span class="hrz-n">'+esc(z.n)+'</span></label>';
    }).join("");
    return '<details class="hrz"'+(tot>0?" open":"")+'><summary>❤️ Zones FC'+(tot>0?' · '+nFmt(tot)+' min':' — minutes par zone')+'</summary>'+
      '<div class="hrz-grid">'+rows+'</div><div class="hrz-live">'+hrBarHTML(hr)+'</div>'+
      '<p class="hrz-help">Repère au ressenti, sans cardio : <b>Z1</b> très facile · <b>Z2</b> tu tiens une conversation en phrases complètes · <b>Z3</b> phrases courtes · <b>Z4</b> quelques mots · <b>Z5</b> rien du tout. Le progrès vient du <b>volume en Z2</b> — le reste ne s\'ajoute qu\'en petite dose.</p>'+
      '</details>';
  }
  function hrAggHTML(id,hist){
    if(!hrTracked(id))return "";
    var agg={},any=0,n=0;
    hist.slice(-8).forEach(function(h){var r=h.rec&&h.rec.hr;if(!r)return;var had=0;
      HR_ZONES.forEach(function(z){var v=num(r[z.k])||0;if(v>0){agg[z.k]=(agg[z.k]||0)+v;any=1;had=1;}});if(had)n++;});
    if(!any)return "";
    return '<div class="hrz-agg"><div class="hrz-agg-t">Répartition FC — '+n+' dernière'+(n>1?"s":"")+' séance'+(n>1?"s":"")+'</div>'+hrBarHTML(agg,"Barres empilées = part du temps dans chaque zone.")+'</div>';
  }
  function sportLogAll(){return state.sportLog||(state.sportLog={});}
  function sportLogGet(id,iso){return sportLogAll()[id+"|"+iso]||null;}
  function sportLogSet(id,iso,rec){
    var k=id+"|"+iso;
    if(rec)sportLogAll()[k]=rec;else delete sportLogAll()[k];
    if(!state.sportSess)state.sportSess={};
    if(rec&&rec.done)state.sportSess[k]=true;else delete state.sportSess[k];
    sportPlanCache=null;save();
  }
  function sportLogHist(id){var m=sportLogAll(),out=[];Object.keys(m).forEach(function(k){var i=k.indexOf("|");if(i>0&&k.slice(0,i)===id){var r=m[k];if(r&&r.done)out.push({iso:k.slice(i+1),rec:r});}});out.sort(function(a,b){return a.iso<b.iso?-1:1;});return out;}
  function sportHistHTML(id){
    var met=sportMetrics(id),pm=met[0],hist=sportLogHist(id);
    if(!hist.length)return '<details class="base-hint"><summary>📊 Historique</summary><div class="exo-hist-empty">Pas encore d\'historique — enregistre une séance, elle s\'affichera ici.</div></details>';
    var vals=hist.map(function(h){return num(h.rec[pm.k])||0;}),mx=Math.max.apply(null,vals.concat([1]))||1;
    var rows=hist.slice(-8).map(function(h){var raw=h.rec[pm.k],v=num(raw)||0,pct=mx>0?Math.max(6,Math.round(v/mx*100)):6;var lbl=(raw!=null&&raw!=="")?(raw+(pm.u?(" "+pm.u):"")):"–";return '<div class="hist-row"><span class="hist-wk">'+esc(frDateShort(h.iso))+'</span><span class="hist-bar-wrap"><span class="hist-bar" style="width:'+pct+'%"></span></span><span class="hist-val">'+esc(lbl)+'</span></div>';}).join("");
    var lr=hist[hist.length-1].rec,parts=met.map(function(f){var v=lr[f.k];return (v!=null&&v!=="")?(f.l+" "+v+(f.u?(" "+f.u):"")):null;}).filter(Boolean);
    if(lr.note)parts.push("« "+lr.note+" »");
    var last=parts.length?'<div class="hist-last">Dernière fois : '+esc(parts.join(" · "))+'</div>':"";
    return '<details class="base-hint" open><summary>📊 Historique</summary><div class="exo-hist">'+rows+'<div class="hist-cap">Barres = '+esc(pm.l.toLowerCase())+' par séance.</div>'+last+hrAggHTML(id,hist)+'</div></details>';
  }
  function sportSessDetailHTML(id,iso,sidx){
    var sp=SPORTS_LIB[id],uc=sportUCfg(id),cur=num(uc.cur)||0,tgt=num(uc.target)||0;
    var paliers=[];for(var lv=cur+1;lv<=tgt;lv++)paliers.push(lv);
    var idx=Math.max(0,Math.min(paliers.length-1,sportLvlNav[id]||0)),lvN=paliers[idx]||tgt;
    var sess=sportSessions(id,lvN),S=sess[sidx]||sess[0]||{title:"Séance",blocks:[]};
    var rec=sportLogGet(id,iso)||{},done=!!rec.done,met=sportMetrics(id);
    var blocksH=(S.blocks&&S.blocks.length)?('<div class="so-prog" style="margin-top:0">'+S.blocks.map(function(b){return '<div class="so-blk"><b>'+esc(b[0])+'</b> — '+esc(b[1])+'</div>';}).join("")+'</div>'):"";
    var fields=met.map(function(f){var v=(rec[f.k]!=null?rec[f.k]:"");return '<div class="set"><span class="sn">'+esc(f.l)+'</span><span class="setf"><input type="'+(f.text?"text":"number")+'"'+(f.text?"":' inputmode="decimal" step="any"')+' class="so-sd-f" data-k="'+esc(f.k)+'" value="'+esc(String(v))+'" placeholder="'+esc(f.ph||"")+'">'+(f.u?'<b>'+esc(f.u)+'</b>':'')+'</span></div>';}).join("");
    var chronoBox=sportSdChrono?('<div class="rest"><div class="rest-disp">0:00</div><div class="rest-btns"><button data-sec="30">30 s</button><button data-sec="45">45 s</button><button data-sec="60">1:00</button><button class="stop" id="restStop">Stop</button></div></div>'):"";
    return '<div class="card pad so-sd" data-sp="'+esc(id)+'" data-iso="'+esc(iso)+'">'+
      '<div class="sd-head"><div><div class="lbl">'+esc(sp.name||id)+' · '+esc(frDateShort(iso))+'</div><h3>'+esc(S.title||"Séance")+'</h3></div>'+
      '<button type="button" class="so-sd-x" aria-label="Fermer">✕</button></div>'+
      blocksH+
      '<label class="so-sd-chrono-t"><input type="checkbox" class="so-sd-chrono"'+(sportSdChrono?" checked":"")+'> ⏱ Minuteur de repos</label>'+chronoBox+
      '<div class="so-sd-log">'+fields+(hrTracked(id)?hrFormHTML(rec.hr||{}):"")+'<textarea class="so-sd-note" rows="2" placeholder="Note (ressenti, météo, technique…)">'+esc(rec.note||"")+'</textarea></div>'+
      '<div class="so-sd-acts"><button type="button" class="btn '+(done?"ghost":"accent")+' so-sd-save">'+(done?"✓ Enregistrer les modifs":"Enregistrer &amp; marquer faite")+'</button>'+(done?'<button type="button" class="btn ghost so-sd-undo">Annuler la séance</button>':'')+'</div>'+
      sportHistHTML(id)+
      '</div>';
  }
  ...
  function renderSportObjectives(){
    ...
    host.querySelectorAll(".so-prev").forEach(function(b){b.onclick=function(){var _k=sdKey();if(_k){var _r=sportLogGet(_k.sid,_k.iso)||{};readSdInto(_r);if(Object.keys(_r).length)sportLogSet(_k.sid,_k.iso,_r);}var id=b.getAttribute("data-sp");sportLvlNav[id]=Math.max(0,(sportLvlNav[id]||0)-1);renderSportObjectives();};});
    host.querySelectorAll(".so-next").forEach(function(b){b.onclick=function(){var _k=sdKey();if(_k){var _r=sportLogGet(_k.sid,_k.iso)||{};readSdInto(_r);if(Object.keys(_r).length)sportLogSet(_k.sid,_k.iso,_r);}var id=b.getAttribute("data-sp"),uc=sportUCfg(id),mx=(num(uc.target)-num(uc.cur))-1;sportLvlNav[id]=Math.min(mx,(sportLvlNav[id]||0)+1);renderSportObjectives();};});
    host.querySelectorAll(".so-log-add").forEach(function(b){b.onclick=function(){var sid=b.getAttribute("data-sp"),iso=todayStr(),key=sid+"|"+iso;if(sportSessOpen===key){sportSessOpen=null;}else{sportSessOpen=key;sportSessOpenIdx=0;sportSdChrono=false;}stopRest();renderSportObjectives();var el=host.querySelector(".so-sd");if(el&&el.scrollIntoView)el.scrollIntoView({behavior:"smooth",block:"nearest"});};});
    host.querySelectorAll(".so-check").forEach(function(b){b.onclick=function(){var sid=b.getAttribute("data-sp"),iso=b.getAttribute("data-iso"),sx=num(b.getAttribute("data-sidx"))||0,key=sid+"|"+iso;if(sportSessOpen===key){sportSessOpen=null;}else{sportSessOpen=key;sportSessOpenIdx=sx;sportSdChrono=false;}stopRest();renderSportObjectives();var el=host.querySelector(".so-sd");if(el&&el.scrollIntoView)el.scrollIntoView({behavior:"smooth",block:"nearest"});};});
    function readSdInto(rec){document.querySelectorAll(".so-sd .so-sd-f").forEach(function(inp){var k=inp.getAttribute("data-k"),v=(inp.value||"").trim();if(v!=="")rec[k]=v;else delete rec[k];});var zs=document.querySelectorAll(".so-sd .so-sd-z");if(zs.length){var hr={};zs.forEach(function(zi){var zv=(zi.value||"").trim();if(zv!=="")hr[zi.getAttribute("data-z")]=zv;});if(hrTot(hr)>0)rec.hr=hr;else delete rec.hr;}var nt=document.querySelector(".so-sd .so-sd-note");if(nt){var nv=(nt.value||"").trim();if(nv)rec.note=nv;else delete rec.note;}return rec;}
    host.querySelectorAll(".so-sd .so-sd-z").forEach(function(zi){zi.oninput=function(){var box=host.querySelector(".so-sd .hrz-live");if(!box)return;var hr={};host.querySelectorAll(".so-sd .so-sd-z").forEach(function(z2){var v=(z2.value||"").trim();if(v!=="")hr[z2.getAttribute("data-z")]=v;});box.innerHTML=hrBarHTML(hr);};});
    function sdKey(){if(!sportSessOpen)return null;var o=sportSessOpen.split("|");return {sid:o[0],iso:o.slice(1).join("|")};}
    var _sdx=host.querySelector(".so-sd-x");if(_sdx)_sdx.onclick=function(){var k=sdKey();if(k){var rec=sportLogGet(k.sid,k.iso)||{};readSdInto(rec);if(Object.keys(rec).length)sportLogSet(k.sid,k.iso,rec);}sportSessOpen=null;stopRest();renderSportObjectives();};
    var _sdc=host.querySelector(".so-sd-chrono");if(_sdc)_sdc.onchange=function(){var k=sdKey();if(k){var rec=sportLogGet(k.sid,k.iso)||{};readSdInto(rec);if(Object.keys(rec).length)sportLogSet(k.sid,k.iso,rec);}sportSdChrono=_sdc.checked;if(!sportSdChrono)stopRest();renderSportObjectives();};
    host.querySelectorAll(".so-sd .rest-btns button[data-sec]").forEach(function(x){x.onclick=function(){startRest(parseInt(x.getAttribute("data-sec"),10));};});
    var _sdstop=host.querySelector(".so-sd #restStop");if(_sdstop)_sdstop.onclick=function(){stopRest();};
    var _sdsave=host.querySelector(".so-sd-save");if(_sdsave)_sdsave.onclick=function(){var k=sdKey();if(!k)return;var rec=sportLogGet(k.sid,k.iso)||{};readSdInto(rec);rec.done=true;rec.date=k.iso;sportLogSet(k.sid,k.iso,rec);sportSessOpen=null;stopRest();renderSportObjectives();if(document.getElementById("homeCal"))renderCalendars();if(typeof renderDayRadar==="function")renderDayRadar();};
    var _sdu=host.querySelector(".so-sd-undo");if(_sdu)_sdu.onclick=function(){var k=sdKey();if(!k)return;var rec=sportLogGet(k.sid,k.iso)||{};readSdInto(rec);rec.done=false;sportLogSet(k.sid,k.iso,rec);sportSessOpen=null;stopRest();renderSportObjectives();if(document.getElementById("homeCal"))renderCalendars();if(typeof renderDayRadar==="function")renderDayRadar();};
    host.querySelectorAll(".so-card[data-sport] > .pane-title").forEach(function(t){t.onclick=function(){var c=t.closest(".so-card"),id=c&&c.getAttribute("data-sport");if(id){paneCollapse[id]=(paneCollapse[id]===false);renderSportObjectives();}};});
    host.querySelectorAll(".grp-title[data-group]").forEach(function(t){t.onclick=function(){var k="grp:"+t.getAttribute("data-group");paneCollapse[k]=(paneCollapse[k]===false);renderSportObjectives();};});
    host.querySelectorAll(".grp-split").forEach(function(b){b.onclick=function(e){e.stopPropagation();delObj(b.getAttribute("data-group"));syncTri();renderSportObjectives();renderCalendars();};});
  ...
  function renderSportTabs(){
    ...
    var mt=document.getElementById("muscuTitle");if(mt){var mDl=(muscuLink()||{}).deadline||"";mt.innerHTML='💪 '+esc(actName("muscu"))+' <button type="button" class="pane-ren" data-ren="muscu" aria-label="Renommer">✎</button>'+(mDl?'<span class="pane-sub'+(dlPast(mDl)?' past':'')+'">'+dlTxt(mDl)+' · '+esc(frDateShort(mDl))+'</span>':'');}
    var tt=document.getElementById("triTitle");if(tt){var tDl=triRaceDate(),_tl=triLink(),tName=(_tl&&_tl.name)||actName("tri");tt.innerHTML='🏊 '+esc(tName)+' <button type="button" class="pane-ren" data-ren="tri" aria-label="Renommer">✎</button>'+(tDl?'<span class="pane-sub'+(dlPast(tDl)?' past':'')+'">'+dlTxt(tDl)+' · '+esc(frDateShort(tDl))+'</span>':'');}
    ...
    if(tt)tt.onclick=function(){paneCollapse.tri=(paneCollapse.tri===false);if(pt)pt.classList.toggle("collapsed",paneCollapse.tri!==false);};
    if(mt){var mr=mt.querySelector(".pane-ren");if(mr)mr.onclick=function(e){e.stopPropagation();var nm=((typeof prompt!=="undefined"?prompt("Nom de l’objectif :",actName("muscu")):"")||"").trim();if(nm){setAct("muscu",{name:nm});var _mlr=muscuLink();if(_mlr)renameObj(_mlr.id,nm);renderSportTabs();}};}
    if(tt){var trn=tt.querySelector(".pane-ren");if(trn)trn.onclick=function(e){e.stopPropagation();var _l=triLink(),cur=(_l&&_l.name)||actName("tri");var nm=((typeof prompt!=="undefined"?prompt("Nom de l’objectif :",cur):"")||"").trim();if(nm){setAct("tri",{name:nm});var l2=triLink();if(l2)renameObj(l2.id,nm);renderSportTabs();}};}
    if(!host)return;
    if(!mOn&&!tOn){host.innerHTML='<p class="hint" style="margin:0">Aucune activité active — active la muscu ou le triathlon dans les Réglages.</p>';return;}
    host.innerHTML="";
  }
  ...
  /* ---------------- En-tête ---------------- */
  function renderChip(){var el=document.getElementById("wkChip");if(!el)return;var n=nextSession();el.textContent=(n?(PROGRAM_BLOCKS[n.block].short+" · S"+n.w):"Fini")+" · "+doneCount()+"/"+totalSessions();}
  ...
  function nextScheduled(){
    ...
    while(g<420){g++;
      var list=sessionsOn(cur);for(var i=0;i<list.length;i++){if(!list[i].done)return {iso:cur,s:list[i]};}
      var sl=(typeof sportSessionsOn==="function")?sportSessionsOn(cur):[];for(var j=0;j<sl.length;j++){if(!sl[j].done)return {iso:cur,s:{kind:"sport",id:sl[j].id,icon:sl[j].icon,name:sl[j].name,label:sl[j].name,disc:null,done:false}};}
  ...
  /* Chips « sports du jour » : rendu dans le meal-head (tous les jours). */
  var SPORT_CHIP_LIST=[
    {id:"course",name:"Course",icon:"\ud83c\udfc3"},
    {id:"velo",name:"V\u00e9lo",icon:"\ud83d\udeb4"},
    {id:"nage",name:"Natation",icon:"\ud83c\udfca"},
    {id:"escalade",name:"Escalade",icon:"\ud83e\uddd7"},
    {id:"tennis",name:"Tennis",icon:"\ud83c\udfbe"},
    {id:"muscu",name:"Muscu",icon:"\ud83d\udcaa"},
    {id:"",name:"Repos",icon:"\ud83d\udecc"},
    {id:"",name:"Autre",icon:"\u2795"}
  ];
  function sportInActiveObjective(id){
    if(!id)return false;
    if(id==="muscu")return actEnabled("muscu");
    if(id==="nage"||id==="velo"||id==="course")return actEnabled("tri");
    var uc=sportUCfg(id);if(uc.on!==false&&num(uc.target)>num(uc.cur))return true;
    return (typeof cfgObjs==="function")&&cfgObjs().some(function(o){return (o.sports||[]).indexOf(id)>=0;});
  }
  function sportLinkHTML(x){
    var linked=[];SPORT_CHIP_LIST.forEach(function(c){if(c.id&&x.sports.indexOf(c.name)>-1&&sportInActiveObjective(c.id))linked.push(c);});
    if(!linked.length)return "";
    var names=linked.map(function(c){return c.name;}).join(", ");
    return '<div class="sob-banner"><span class="sob-tx">\ud83c\udfaf '+esc(names)+' fait partie d\'un objectif en cours \u2014 ouvre le programme pour remplir la s\u00e9ance.</span><button type="button" class="sob-go" data-spid="'+esc(linked[0].id)+'">Ouvrir l\'objectif \u2192</button></div>';
  }
  function sportChipsHTML(x){
    var chips=SPORT_CHIP_LIST.map(function(c){var on=x.sports.indexOf(c.name)>-1;return '<button type="button" class="chip'+(on?' on':'')+'" data-sport="'+esc(c.name)+'" data-spid="'+esc(c.id||"")+'">'+esc((c.icon?c.icon+" ":"")+c.name)+'</button>';}).join("");
    var otherOn=x.sports.indexOf("Autre")>-1;
    var other=otherOn?('<input type="text" class="sport-other" placeholder="Pr\u00e9cise le sport\u2026" value="'+esc(x.sportOther||"")+'">'):"";
    return '<div class="chips">'+chips+'</div>'+other+sportLinkHTML(x);
  }
  var SPORT_OF_DISC={nat:"Natation",velo:"Vélo",course:"Course"};
  /* Lien programme → tâche effectuée : valider une séance ajoute le sport correspondant au
     « sport du jour » de SA date. Ajout seul — jamais de retrait auto (saisie manuelle préservée).
     Le sens inverse n'est pas fait : une pastille est grossière, une séance a une cible précise. */
  function linkSessionSport(sport,iso){if(!sport||!iso)return;var xx=day(iso);if(xx.sports.indexOf(sport)<0)xx.sports.push(sport);}

  /* Phase 4 — conseil nutrition adapté à l'entraînement du jour (advisory, ne touche pas les objectifs). */
  function trainLoadToday(){
    var d=todayStr(),strength=false,endur=false,n=0;
    (sessionsOn(d)||[]).forEach(function(s){n++;if(s.kind==="muscu")strength=true;else endur=true;});
    if(typeof sportSessionsOn==="function")(sportSessionsOn(d)||[]).forEach(function(s){n++;if(s.id==="muscu")strength=true;else if(["nage","velo","course"].indexOf(s.id)>=0)endur=true;else strength=strength;});
    var lg=(day(d).sports||[]);
    if(lg.indexOf("Muscu")>=0)strength=true;
    ["Natation","Vélo","Course"].forEach(function(x){if(lg.indexOf(x)>=0)endur=true;});
    var extra=lg.filter(function(x){return ["Muscu","Natation","Vélo","Course","Repos","Autre"].indexOf(x)<0;}).length;
    n+=lg.length;
    return {strength:strength,endur:endur,any:(strength||endur||extra>0),big:(n>=3)};
  }
  function renderNutriAdapt(){
    var host=document.getElementById("nutriAdapt");if(!host)return;
    var L=trainLoadToday(),ic,msg;
    if(!L.any){ic="🛌";msg="Journée sans séance prévue — énergie normale. Garde le cap sur tes protéines pour la récupération.";}
    else if(L.big){ic="🔥";msg="Grosse journée (plusieurs séances) — monte l'énergie via les glucides, hydrate-toi bien, et vise le haut de ta fourchette protéines.";}
    else if(L.strength&&L.endur){ic="💪🏃";msg="Muscu + endurance aujourd'hui — glucides autour des séances pour l'énergie, protéines réparties sur la journée, et hydratation.";}
    else if(L.strength){ic="💪";msg="Journée muscu — priorité protéines réparties sur la journée, et des glucides autour de la séance pour l'énergie.";}
    else{ic="🏃";msg="Journée endurance — glucides avant/pendant l'effort, protéines pour la récup, et bien t'hydrater.";}
    host.innerHTML='<div class="nuta-card"><span class="nuta-ic">'+ic+'</span><div class="nuta-tx"><div class="nuta-h">Nutrition du jour</div><div class="nuta-m">'+esc(msg)+'</div></div></div>';
  }
  function renderHero(){
    ...
    var s=next.s, muscu=(s.kind==="muscu"), sport=(s.kind==="sport");
    var icon=sport?(s.icon||"•"):(muscu?"🏋️":triIcon(s.disc));
    var when=whenLabel(next.iso);
    var body="";
    if(bndOpen.hero){
      if(sport){
        body='<div class="stitle">Progression sport</div>'+
             '<h2>'+esc(s.icon||"")+' '+esc(s.name)+'</h2>'+
             '<div class="meta">'+when+'</div>'+
             '<div class="row2"><button class="btn accent" id="quickDoneSport">✓ Marquer faite</button></div>';
      }else if(muscu){var blk=PROGRAM_BLOCKS[s.block],p=progOf(s.block,s.code);
    ...
    if(bndOpen.hero){
      if(sport){
        var qds=hero.querySelector("#quickDoneSport");if(qds)qds.onclick=function(){toggleSportSess(s.id,next.iso);renderChip();renderHero();renderCalendars();renderDayRadar();};
      }else if(muscu){
        ...
        var qd=hero.querySelector("#quickDone");if(qd)qd.onclick=function(){var r=sess(s.block,s.w,s.code);r.done=true;if(!r.date)r.date=todayStr();linkSessionSport("Muscu",r.date);save();renderChip();renderHero();renderCalendars();};
      }else{
        ...
        var qt=hero.querySelector("#quickDoneTri");if(qt)qt.onclick=function(){var k=s.w+"_"+s.disc;var r=state.tri[k]||(state.tri[k]={});r.done=true;if(!r.date)r.date=todayStr();linkSessionSport(SPORT_OF_DISC[s.disc],r.date);save();renderChip();renderHero();renderCalendars();};
      }
    }
  }
  ...
  function renderDayBalance(d,host){
    if(!host)return;
    var b=bmr(),intake=adjIntake(d),lv=actLevel(d),out=expend(d),net=(out!=null)?(intake-out):null;
    ...
    host.querySelectorAll(".bal-lvl").forEach(function(bt){bt.onclick=function(){day(d).actLvl=bt.getAttribute("data-lvl");save();renderDayBalance(d,host);};});
    host.querySelectorAll(".bal-a-d,.bal-a-t").forEach(function(inp){inp.onchange=function(){var kind=inp.getAttribute("data-act"),dd=day(d);if(!dd.act)dd.act={};if(!dd.act[kind])dd.act[kind]={d:"",t:""};dd.act[kind][inp.classList.contains("bal-a-d")?"d":"t"]=inp.value;save();renderDayBalance(d,host);};});
    var wi=host.querySelector(".bal-watch-in");if(wi)wi.onchange=function(){var dd=day(d),v=wi.value.trim();if(v==="")delete dd.actKcal;else dd.actKcal=v;save();renderDayBalance(d,host);};
  }
  ...
  function pfRefresh(){axDetailRefresh();var h=document.getElementById("dayLog");if(h&&h.innerHTML)buildDayForm(h,dayDate);renderDayRadar();}
  ...
  function renderDayNutri(d,host){
    if(!host)return;
    var isTd=(d===todayStr());
    var tot=dayTotals(d);
    ...
    host.querySelectorAll(".md-mode").forEach(function(b){b.onclick=function(){mealDistMode=b.getAttribute("data-mode");renderDayNutri(d,host);};});
    host.querySelectorAll(".pf-row").forEach(function(b){b.onclick=function(){pfAdd(b.getAttribute("data-pf"),b.getAttribute("data-q"));};});
    var pu=host.querySelector(".pf-undo");if(pu)pu.onclick=function(){pfUndo();};      var tg=host.querySelector(".nutri-tgtg"),tp=host.querySelector(".tgtg-panel");if(tg&&tp)wireTgtg(tg,tp,d,function(){renderDayNutri(d,host);buildDayForm(document.getElementById("dayLog"),d);});
  }
  ...
  function radarDay(d){
    ...
    var sportT=(typeof sportSessionsOn==="function")?sportSessionsOn(d).filter(function(s){return s.done;}).length:0;
    var sportU=sportsN+muscuT+triT+sportT;
    ...
    if(exp!=null){
      ...
      var tol=Math.abs(goal)||300,gap=Math.abs(net-goal),rr=1-gap/tol;
  ...
  function renderDayRadar(){
    var h=document.getElementById("dayRadar");if(!h)return;
    if(!bndOpen.radar){h.innerHTML="";renderDayScoreChip();return;}
    var A=radarDay(dayDate),overall=Math.round(A.reduce(function(s,a){return s+a.r;},0)/A.length*100);
    ...
    h.innerHTML='<div class="card pad radar-open"><div class="radar-body">'+radarSVG(A,overall)+
      '<button type="button" class="radar-det'+(radarDetail?' open':'')+'">D\u00e9tail<span class="bnd-chev">\u25be</span></button>'+
      (radarDetail?('<div class="radar-legend">'+legend+'</div><div class="radar-sub">R\u00e9alis\u00e9 vs objectif \u2014 plus la forme touche le bord, plus ta journ\u00e9e est compl\u00e8te.</div>'):'')+'</div></div>';
    var dt=h.querySelector(".radar-det");if(dt)dt.onclick=function(){radarDetail=!radarDetail;renderDayRadar();};
    renderDayScoreChip();
  }
  ...
  /* Détail du jour de l'écran d'axe : SEUL endroit où vivent les cartes
     Protéines et Bilan kcal en entier (le formulaire du jour n'en montre que
     le résumé). Point d'entrée unique pour les re-rendre après une écriture. */
  function axDetailRefresh(){
    if(!axShown)return;
    var slot=document.querySelector("#axisBody .ax-dslot");if(!slot)return;
    var d=axDate||todayStr();
    if(axLab==="Prot\u00e9ines")renderDayNutri(d,slot);
    else if(axLab==="Bilan kcal")renderDayBalance(d,slot);
  }
  ...
  function syncStickTop(){
    ...
    var base=h+(dh>0?dh+6:48);
    r.style.setProperty("--bnd-top2",base+"px");
  }
  function wireStick(){
    var tick=false;
    function upd(){
  ...
  function renderDay(){
    ...
    if(dl){dl.textContent=frDateFull(d)+(isTd?" \u00b7 aujourd'hui":"");dl.classList.toggle("past",!isTd);dl.classList.toggle("tap-today",!isTd);}
  ...
  /* Pesée : la cible est 2 fois par semaine (7 jours glissants), pas tous les
     jours — une pesée quotidienne mesure surtout le contenu de l'intestin. */
  function weightLogs7(){var n=0;for(var i=0;i<7;i++){var y=state.days[isoOf(addDays(todayStr(),-i))];if(y&&(""+(y.weight||"")).trim()!=="")n++;}return n;}
  /* Tous les raccourcis « m'emmener là où ça se saisit » passent par ici :
     bandeaux à ouvrir + sélecteur du champ visé. Utilisé par les alertes ET
     par les emojis de Régularité (Progrès). */
  var JUMP={
    supps:{o:["wb","sp"],s:'[data-bnd="sp"]'},
    ...
    water:{o:["water"],s:".hydra"},
    prot:{o:[],s:".meal-head"}
  };
  function jumpDay(kind){
    var j=JUMP[kind];if(!j)return;
    j.o.forEach(function(k){bndOpen[k]=true;});
    dayDate=todayStr();activateTab("v-day");
    var h=document.getElementById("dayLog");if(!h)return;
    buildDayForm(h,dayDate); /* l'\u00e9tat vient d'\u00eatre chang\u00e9 : il faut reg\u00e9n\u00e9rer, sinon rien ne s'ouvre */
    var el=h.querySelector(j.s);if(el&&el.scrollIntoView)el.scrollIntoView({block:"center"});
  }
  ...
  function buildGrid(block){
    ...
    var _cd=codesOf(block),doneN=0,totN=blk.weeks*_cd.length,ww,ii;
    for(ww=1;ww<=blk.weeks;ww++)for(ii=0;ii<_cd.length;ii++)if(sess(block,ww,_cd[ii]).done)doneN++;
  ...
  function gridBody(block){
    var blk=PROGRAM_BLOCKS[block],n=nextSession(),_cd=codesOf(block);
    var h='<div class="legend"><span><i class="dot-next"></i>Prochaine</span><span><i class="dot-done"></i>Faite</span><span><i class="dot-todo"></i>À faire</span></div>'+
      '<table class="grid"><tr><th></th>';
    _cd.forEach(function(c){h+="<th>"+c+"</th>";});
    h+="</tr>";
    for(var w=1;w<=blk.weeks;w++){
      h+='<tr><td class="wk">S'+w+'<span class="wkd">'+muscuWeekDate(block,w)+'</span></td>';
      for(var i=0;i<_cd.length;i++){
        var c=_cd[i],s=sess(block,w,c),cls="cell";
        if(s.done)cls+=" done";
        if(n&&n.block===block&&n.w===w&&n.c===c)cls+=" next";
        if(currentSel&&currentSel.block===block&&currentSel.w===w&&currentSel.c===c)cls+=" sel";
        h+='<td><div class="'+cls+'" data-b="'+block+'" data-w="'+w+'" data-c="'+c+'">'+c+(s.done?'<span class="chk">✓</span>':'')+'</div></td>';
  ...
  function muscuSessListHTML(){
    return '<p class="set-note">Modifie durablement tes séances : renomme, ajoute, retire ou ajuste les exercices et les séries.</p>'+
      BLOCK_ORDER.map(function(b){return '<div class="sess-blk">'+esc(PROGRAM_BLOCKS[b].name)+'</div>'+
        codesOf(b).map(function(c){var p=progOf(b,c);var cust=progIsCustom(b,c);
          return '<div class="set-row sess-pick" data-sess="'+b+'_'+c+'"><span class="set-ic">💪</span><span class="set-main"><span class="set-lbl">Séance '+c+(cust?' <span class="sess-badge">modifiée</span>':'')+'</span><span class="set-sub">'+esc(p.title.replace(/^S[eé]ance [A-Z]\s*—\s*/,""))+' · '+(p.exos?p.exos.length:0)+' exos</span></span><span class="sess-arrow">›</span></div>';
        }).join("");
      }).join("");
  }
  ...
  function renderProgram(){
    ...
    var block=BLOCK_ORDER[muscuLvl],blk=PROGRAM_BLOCKS[block],doneN=blockDone(block),totN=blockTotal(block),done=doneN>=totN;
    ...
    if(currentSel)renderSessionDetail();else{var sd=document.getElementById("sessionDetail");if(sd)sd.innerHTML="";}
  }
  ...
  function renderSessionDetail(){
    ...
    p.exos.forEach(function(ex){
      ...
      var isMin=ex.unit==="min";                    /* endurance : une durée par bloc, pas de charge */
      var isSec=(ex.unit==="sec"||isMin);
      var perSide=/\/côté/.test(ex.target||"");
      var secLbl=isMin?"min":(perSide?"s/côté":"s");
      var secTgt=(String(ex.target).match(isMin?/(\d+)\s*min/:/(\d+)\s*s/)||[])[1]||(isMin?"min":"s");
    ...
    wrap.querySelector("#toggleDone").addEventListener("click",function(){s.done=!s.done;if(s.done&&!s.date)s.date=todayStr();if(s.done)linkSessionSport("Muscu",s.date);save();renderProgram();renderSessionDetail();});
  ...
  /* Agrégat des zones FC sur les séances tri d'une discipline (toutes semaines loguées). */
  function triHrAgg(dz){var agg={},n=0;for(var i=1;i<=TRI.length;i++){var r=state.tri[i+"_"+dz];if(r&&r.hr&&hrTot(r.hr)>0){n++;HR_ZONES.forEach(function(z){var v=num(r.hr[z.k])||0;if(v>0)agg[z.k]=(agg[z.k]||0)+v;});}}return {agg:agg,n:n};}
  ...
  function renderTriProgress(){
    ...
    var _TT=triTargets();var rows=TRI_DISC.map(function(p){var dz=p[0],tg=_TT[dz],b=triBest(dz),pct=tg.v>0?Math.min(100,Math.round(b/tg.v*100)):0;
    ...
    var _rd=triRaceDate(),over=dlPast(_rd);
    var banner=over?('<div class="card pad race-over"><div class="ro-h">🏁 Course passée — '+esc(frDateShort(_rd))+' ('+dlTxt(_rd)+')</div><p class="ro-p">Le plan ci-dessous est <b>terminé</b> : il reste consultable, tes séances loguées sont intactes. Tant qu\'aucune nouvelle échéance n\'est fixée, tu es en <b>cycle ouvert</b> — on entretient le moteur (volume facile) plutôt que de viser une date.</p></div>'):"";
    host.innerHTML=banner+'<details class="card pad tp-card tp-fold"><summary class="tp-head">Vers la distance olympique 🏁</summary>'+rows+'<div class="tp-note">Meilleure distance loguée par discipline (renseigne distance + durée dans une séance).</div></details>';
  }
  function renderTriDetail(){
    ...
    wrap.innerHTML=
      ...
      '<div class="sd-head"><div><div class="lbl">Semaine '+currentTri.w+(wk.phase?' · '+wk.phase:(wk.nat.taper?' · affûtage':''))+'</div><h3>'+names[dz]+'</h3></div><span class="tg">'+info.t+'</span></div>'+        '<p class="muted" style="margin-top:10px">'+info.d+'</p>'+
        ...
        '<div class="field"><label>Réalisé</label><div class="tri-io"><input type="number" inputmode="decimal" step="0.1" min="0" class="t-dist" placeholder="'+(dz==="nat"?"ex : 1300":"ex : "+(dz==="velo"?"32":"7,5"))+'"><span class="tri-u">'+triTargets()[dz].u+'</span><input type="number" inputmode="decimal" step="1" min="0" class="t-dur" placeholder="min"><span class="tri-u">min</span></div><div class="tri-pace" hidden></div>'+(rec.val?'<div class="tri-legacy">Ancien réalisé : '+esc(rec.val)+'</div>':'')+'</div>'+
        '<div class="field">'+hrFormHTML(rec.hr||{})+(function(){var a=triHrAgg(dz);return a.n>1?('<div class="hrz-agg"><div class="hrz-agg-t">Répartition FC — '+a.n+' séances '+esc(names[dz].toLowerCase())+'</div>'+hrBarHTML(a.agg,"Barres empilées = part du temps dans chaque zone.")+'</div>'):"";})()+'</div>'+
    ...
    wrap.querySelectorAll(".so-sd-z").forEach(function(zi){zi.addEventListener("input",function(){
      var hr={};wrap.querySelectorAll(".so-sd-z").forEach(function(z2){var v=(z2.value||"").trim();if(v!=="")hr[z2.getAttribute("data-z")]=v;});
      if(hrTot(hr)>0)rec.hr=hr;else delete rec.hr;save();
      var box=wrap.querySelector(".hrz-live");if(box)box.innerHTML=hrBarHTML(rec.hr||{});});});
    ...
    wrap.querySelector("#triDone").addEventListener("click",function(){rec.done=!rec.done;if(rec.done&&!rec.date)rec.date=todayStr();if(rec.done)linkSessionSport(SPORT_OF_DISC[dz],rec.date);save();renderTri();renderTriDetail();});
    var tdd=wrap.querySelector("#triDoneDate");if(tdd)tdd.addEventListener("change",function(){if(this.value){rec.date=this.value;if(rec.done)linkSessionSport(SPORT_OF_DISC[dz],rec.date);save();renderTri();}});
  }
  ...
  function dfWater(x,d){
    ...
    return '<div class="field hydra">'+
      '<div class="hy-top"><label class="hy-lab" title="Hydratation">\ud83d\udca7</label>'+wAddHint()+
        '<details class="base-hint inl wadd-d"'+(waddOpen?" open":"")+'><summary>'+(wAddEmo(x).trim()||"+")+'</summary><div>'+wAddHTML(x)+'</div></details>'+
        '<span class="hy-sum"></span></div>'+
...
'<div class="field"><span class="lbl-row"><label>VFC au r\u00e9veil (ms)</label><details class="base-hint inl"><summary>i</summary><div>La VFC (variabilit\u00e9 de la fr\u00e9quence cardiaque, en ms) mesure les micro-\u00e9carts entre deux battements de c\u0153ur. Plus elle est haute, mieux ton syst\u00e8me nerveux r\u00e9cup\u00e8re : bon indicateur de fatigue r\u00e9elle plut\u00f4t que ressentie. Une VFC basse le matin = corps encore fatigu\u00e9, tu peux all\u00e9ger la s\u00e9ance ou viser la r\u00e9cup\u00e9ration. Pour la relever : ta montre (Apple Watch \u2192 app Sant\u00e9 \u2192 Variabilit\u00e9 de la FC) la mesure la nuit ; note la valeur en ms chaque matin, au calme, pour comparer jour apr\u00e8s jour.</div></details></span><input type="number" inputmode="numeric" step="1" class="f-hrv" placeholder="ex : 52"><div class="hrv-trend"></div></div>'+
  ...
  function dfRepas(x,d){
    ...
    return '<div class="field"><div class="meal-head">'+
          ...
          MEALS.map(function(m){return '<div class="meal'+(mealOpen[m.k]?" open":"")+'" data-mk="'+m.k+'"><div class="meal-hrow"><button type="button" class="meal-h" data-mk="'+m.k+'"><span class="meal-lbl">'+m.label+'</span><span class="meal-sum" data-sum="'+m.k+'"></span><span class="meal-chev">▾</span></button><button type="button" class="meal-supp" data-mk="'+m.k+'" aria-label="Compléments pris à ce repas">💊<span class="ms-badge" data-msb="'+m.k+'"></span></button></div><div class="meal-items" data-mk="'+m.k+'"></div><div class="meal-supps collapsed" data-mk="'+m.k+'"></div></div>';}).join("")+
  ...
  function dfBienEtre(x,d){
    return '<div class="field supps-field wb-group">'+
        ...
        '<div class="field supps-field">'+
          bndHead("rx","sub",{ttl:"☀️ Ce qui te fait du bien",cls:"rx-meta",meta:""})+
  ...
  function buildDayForm(container,d){
    ...
    (function(){
      /* Résumé d'en-tête tenu à jour sans toucher aux handlers existants — et,
         dans la foulée, le radar (le graphe araignée reflète la saisie en direct).
         Débounce : on ne re-rend pas le radar à chaque frappe dans le champ Poids. */
      var pt=null;
      function pulse(){
        dgMeta();
        if(pt)clearTimeout(pt);
        pt=setTimeout(function(){pt=null;renderDayRadar();},220);
      }
      container.addEventListener("input",pulse);
      container.addEventListener("change",pulse);
      container.addEventListener("click",function(){setTimeout(pulse,0);});
    })();
    dgMeta();
    ...
    (function(){var ka=container.querySelector(".f-kcaladj");if(!ka)return;ka.addEventListener("change",function(){var v=(ka.value||"").trim();if(v==="")delete x.kcalAdj;else x.kcalAdj=v;save();buildDayForm(container,d);});})();
    ...
    function renderSpday(){var host=container.querySelector(".spday-body");if(!host)return;host.innerHTML=sportChipsHTML(x);wireSpday();}
    function wireSpday(){var host=container.querySelector(".spday-body");if(!host)return;
      host.querySelectorAll(".chip").forEach(function(ch){ch.addEventListener("click",function(){var sp=ch.getAttribute("data-sport");var arr=x.sports;var i=arr.indexOf(sp);if(i>-1)arr.splice(i,1);else arr.push(sp);save();renderDayRadar();renderSpday();});});
      var oi=host.querySelector(".sport-other");if(oi)oi.oninput=function(){x.sportOther=oi.value;save();};
      var go=host.querySelector(".sob-go");if(go)go.onclick=function(){goSport(go.getAttribute("data-spid")||"muscu");};
    }
    wireSpday();
    ...
    (function(){
      ...
      var wd=container.querySelector(".wadd-d");
      if(wd)wd.addEventListener("toggle",function(){waddOpen=wd.open;});
    ...
    function recalcTotals(){var t=dayTotals(d);
      /* ⚠️ Aliments à chiffrer : le toggle n'apparaît que s'il y en a ; son dépliant les liste,
         un tap ouvre l'éditeur de l'aliment (rubrique « Composition » dépliée). */
      var _list=[];MEALS.forEach(function(m){((day(d).mealItems||{})[m.k]||[]).forEach(function(it,i){if(itemTodo(it))_list.push({mk:m.k,i:i,name:it.name||"\u2014"});});});
      var _ct=container.querySelector('[data-bnd="chiffr"]'),_cb=container.querySelector(".chiffr-body");
      if(_ct){_ct.hidden=!_list.length;
        if(!_list.length){bndOpen.chiffr=false;_ct.classList.remove("open");if(_cb)_cb.classList.add("collapsed");}
        if(_cb){_cb.innerHTML='<div class="chiffr-hint">Ces aliments comptent pour 0 tant qu\'ils n\'ont pas de valeurs. Touche-en un pour les renseigner.</div>'+_list.map(function(o){return '<button type="button" class="chiffr-it" data-mk="'+o.mk+'" data-i="'+o.i+'">\u26a0\ufe0f '+esc(o.name)+'</button>';}).join("");
          _cb.querySelectorAll(".chiffr-it").forEach(function(b){b.onclick=function(){gotoTodo(b.getAttribute("data-mk"),parseInt(b.getAttribute("data-i"),10));};});}}
      var jt=container.querySelector('[data-bnd="jprot"]'),jb=container.querySelector(".jprot-body");if(jt&&jb){if(t){jt.hidden=false;jb.innerHTML=protBreakHTML(t)+dayMealDistHTML(d);jb.querySelectorAll(".md-mode").forEach(function(b){b.onclick=function(){mealDistMode=b.getAttribute("data-mode");recalcTotals();};});}else{jt.hidden=true;jb.innerHTML="";}}}
    /* Deux défauts SILENCIEUX repérés sur les données réelles :
       — un aliment sans valeurs compte pour ZÉRO sans rien dire (31 items sur
         5 semaines : totaux sous-estimés sans le moindre signal) ;
       — une quantité aberrante passe sans broncher (600 g d'escalope en
         collation = 180 g de protéines sur la journée).
       Les deux se voient désormais sur l'étiquette de l'aliment. */
    function itemTodo(it){return !nutFilled(it);}
    function itemFlag(it){
      if(itemTodo(it))return {c:"todo",ic:"\u26a0\ufe0f",
        t:"Valeurs nutritionnelles manquantes \u2014 cet aliment compte pour 0 dans tes totaux. Touche-le pour les renseigner."};
      var sc=scaleNut(it);
      if(sc&&((sc.kcal||0)>3500||(sc.prot||0)>250))return {c:"big",ic:"\u2757",
        t:"Valeur aberrante pour un seul aliment : "+Math.round(sc.kcal||0)+" kcal \u00b7 "+fr1(sc.prot||0)+" g prot. \u2014 probable erreur d'unit\u00e9 ou de quantit\u00e9, touche-le pour corriger."};
      return null;
    }
    var mealEdit={pd:-1,dj:-1,dn:-1,co:-1},mealEditPrev=null;
    /* Depuis le dépliant ⚠️ : ouvre le repas concerné, sélectionne l'aliment et déplie sa
       rubrique Composition (renderMeal force tenut ouvert quand les valeurs manquent). */
    function gotoTodo(mk,i){
      mealOpen[mk]=true;mealEdit[mk]=i;mealEditPrev=null;
      var md=container.querySelector('.meal[data-mk="'+mk+'"]');if(md)md.classList.add("open");
      renderMeal(mk);
      var host=container.querySelector('.meal-items[data-mk="'+mk+'"]');if(host&&host.scrollIntoView)host.scrollIntoView({block:"center"});
    }
    function renderMealSupps(mk){
      var host=container.querySelector('.meal-supps[data-mk="'+mk+'"]');if(!host)return;
      var xx=day(d),open=!!mealSuppOpen[mk],S=(typeof SUPPS!=="undefined"?SUPPS:[]);
      host.classList.toggle("collapsed",!open);
      var cnt=0;S.forEach(function(sp){if(xx.supps&&xx.supps[sp.id]&&(((xx.suppMeal&&xx.suppMeal[sp.id])||"co")===mk))cnt++;});
      var badge=container.querySelector('.ms-badge[data-msb="'+mk+'"]');if(badge)badge.textContent=cnt?cnt:"";
      if(!open){host.innerHTML="";return;}
      host.innerHTML='<div class="ms-list">'+S.map(function(sp){
        var on=!!(xx.supps&&xx.supps[sp.id]&&(((xx.suppMeal&&xx.suppMeal[sp.id])||"co")===mk));
        var x2=!!(xx.supps2&&xx.supps2[sp.id]);
        return '<label class="ms-it'+(on?" on":"")+'"><input type="checkbox" class="ms-cb" data-id="'+sp.id+'"'+(on?" checked":"")+'>'+
          '<span class="ms-name">'+esc(sp.name)+(sp.dose?' <span class="ms-dose">'+esc(sp.dose)+'</span>':"")+'</span>'+
          (sp.prot?'<span class="ms-badge2">+'+sp.prot+' g</span>':"")+
          '<button type="button" class="ms-x2'+(x2?" on":"")+'" data-x2="'+sp.id+'" title="Pris 2 fois">×2</button></label>';
      }).join("")+'</div><div class="ms-hint">Coché = pris à ce repas. Le whey compte dans tes protéines.</div>';
      host.querySelectorAll(".ms-cb").forEach(function(cb){cb.addEventListener("change",function(){var id=cb.getAttribute("data-id");if(!xx.supps)xx.supps={};if(!xx.suppMeal)xx.suppMeal={};xx.supps[id]=cb.checked;if(cb.checked)xx.suppMeal[id]=mk;save();renderMealSupps(mk);renderMeal(mk);recalcTotals();renderDayRadar();});});
      host.querySelectorAll(".ms-x2").forEach(function(bt){bt.addEventListener("click",function(e){e.preventDefault();e.stopPropagation();var id=bt.getAttribute("data-x2");if(!xx.supps2)xx.supps2={};xx.supps2[id]=!xx.supps2[id];bt.classList.toggle("on",xx.supps2[id]);save();renderMealSupps(mk);if(xx.supps&&xx.supps[id]){renderMeal(mk);recalcTotals();renderDayRadar();}});});
    }
    function renderMeal(mk){
      ...
      arr.forEach(function(it,i){
        var _fl=itemFlag(it),_qd=!(num(it.qty)>0);
        h+='<span class="tag'+(ed===i?" on":"")+(it.nut?" has-nut":"")+(_fl?" fl-"+_fl.c:"")+'" data-i="'+i+'">'+esc(it.name||"\u2014")+foodQualityBadges(it.name,d)+(_fl?'<span class="tag-warn" title="'+esc(_fl.t)+'">'+_fl.ic+'</span>':"")+(it.nut?'<span class="tag-q'+(_qd?" def":"")+'" title="'+(_qd?"Portion par d\u00e9faut \u2014 aucune quantit\u00e9 saisie":"")+'">'+(_qd?"~":"")+esc(effPortion(it))+'</span>':'')+'<button type="button" class="tag-x" data-i="'+i+'" aria-label="Supprimer">\u00d7</button></span>';
      });
      h+='</div>';
      ...
      if(ed>-1&&arr[ed]){
        ...
        h+='<div class="tag-editor"><div class="te-title">'+(teRenaming?('<input class="te-name-in" value="'+esc(it.name)+'" aria-label="Renommer l\'aliment"><button type="button" class="te-name-ok" aria-label="Valider">✓</button>'):('<span class="te-name">'+esc(it.name)+'</span><button type="button" class="te-rename" aria-label="Renommer" title="Renommer">✏️</button>'))+'</div>'+
          ...
          '<div class="te-foot"><button type="button" class="te-fiche" aria-label="Fiche produit (r\u00e9glages)" title="Fiche produit (r\u00e9glages)">\u2699\ufe0f</button><button type="button" class="te-close">Fermer</button></div>'+
        '</div>';
      }
      host.innerHTML=h;
      var _sum=container.querySelector('.meal-sum[data-sum="'+mk+'"]');if(_sum){var _a=day(d).mealItems[mk]||[],_p=0;_a.forEach(function(it){var s=scaleNut(it);if(s&&s.prot)_p+=s.prot;});(typeof SUPPS!=="undefined"?SUPPS:[]).forEach(function(sp){var xx=day(d);if(xx.supps&&xx.supps[sp.id]&&sp.prot&&(((xx.suppMeal&&xx.suppMeal[sp.id])||"co")===mk))_p+=sp.prot*((xx.supps2&&xx.supps2[sp.id])?2:1);});var _t=0;_a.forEach(function(it){if(itemTodo(it))_t++;});
        _sum.textContent=fr1(_p)+" / "+Math.round(130/MEALS.length)+" g prot"+(_p>=Math.round(130/MEALS.length)?" ✓":"")+(_t?(" \u00b7 "+_t+" \u00e0 chiffrer"):"");}
      host.querySelectorAll(".tag").forEach(function(tg){
        tg.addEventListener("click",function(e){if(e.target.classList.contains("tag-x")||(e.target.closest&&e.target.closest(".fq")))return;var i=parseInt(tg.getAttribute("data-i"),10);teRenaming=false;mealEdit[mk]=(mealEdit[mk]===i?-1:i);renderMeal(mk);});
      });
      ...
      if(ed>-1&&arr[ed]){
        ...
        host.querySelector(".te-close").addEventListener("click",function(){teRenaming=false;mealEdit[mk]=-1;renderMeal(mk);});
        var teRen=host.querySelector(".te-rename");
        if(teRen)teRen.addEventListener("click",function(){teRenaming=true;renderMeal(mk);var inp=host.querySelector(".te-name-in");if(inp){inp.focus();inp.select&&inp.select();}});
        var teNameIn=host.querySelector(".te-name-in");
        if(teNameIn){
          var commit=function(){var v=(teNameIn.value||"").trim();if(v)item.name=v;teRenaming=false;save();renderMeal(mk);};
          teNameIn.addEventListener("keydown",function(e){if(e.key==="Enter"){e.preventDefault();commit();}else if(e.key==="Escape"){teRenaming=false;renderMeal(mk);}});
          var teOk=host.querySelector(".te-name-ok");if(teOk)teOk.addEventListener("click",commit);
        }
    ...
    MEALS.forEach(function(m){renderMeal(m.k);renderMealSupps(m.k);});
    container.querySelectorAll(".meal-h").forEach(function(hh){hh.onclick=function(){var mk=hh.getAttribute("data-mk");mealOpen[mk]=!mealOpen[mk];var meal=hh.closest(".meal");if(meal)meal.classList.toggle("open",!!mealOpen[mk]);};});
    container.querySelectorAll(".meal-supp").forEach(function(bb){bb.onclick=function(){var mk=bb.getAttribute("data-mk");mealSuppOpen[mk]=!mealSuppOpen[mk];renderMealSupps(mk);};});
    recalcTotals();
  }
  ...
  function renderGoals(){
    ...
    function goal(title,sub,done,total,dleft,go){
      ...
      var badge=(dleft===null||dleft===undefined)?'\u203a':(dtxt(dleft)+' \u203a');
      return '<div class="goal goal-click" data-goto="'+go+'">'+
        '<div class="goal-head"><div class="goal-name">'+title+'</div><span class="goal-badge">'+badge+'</span></div>'+
    ...
    function objProg(o){
      if(o.id==="muscu"){var dn=0,tt=0;BLOCK_ORDER.forEach(function(b){dn+=blockDone(b);tt+=blockTotal(b);});return {done:dn,total:tt};}
      if(o.id==="tri")return {done:triDoneCount(),total:30};
      if(o.sports&&o.sports.length){var dn=0,tt=0;o.sports.forEach(function(sid){if(typeof sportDoneCount==="function")dn+=sportDoneCount(sid);var uc=sportUCfg(sid);tt+=Math.max(0,(num(uc.target)-num(uc.cur)))*8;});return {done:dn,total:tt};}
      return {done:0,total:0};
    }
    function goto1(o){if(o.id==="muscu")return "muscu";if(o.id==="tri")return "tri";return (o.sports&&o.sports[0])||"muscu";}
    function spNames(o){return (o.sports||[]).map(function(s){return (typeof SPORTS_LIB!=="undefined"&&SPORTS_LIB[s])?SPORTS_LIB[s].name:s;}).join(" \u00b7 ");}
    var objs=objectives();
    if(!objs.length){host.innerHTML="";return;}
    var sportO=objs.filter(function(o){return o.id==="muscu"||o.id==="tri"||o.kind==="sport"||(o.sports&&o.sports.length);});
    var otherO=objs.filter(function(o){return sportO.indexOf(o)<0;});
    var h='<div class="card pad"><div class="sec-title">Objectifs</div>';
    if(sportO.length){h+='<div class="goals-tag">\ud83c\udfc5 Sport</div>'+sportO.map(function(o){var p=objProg(o);var sub=o.deadline?frDateShort(o.deadline):(spNames(o)||esc(actName(o.id)));return goal(esc(o.icon)+" "+esc(o.name),sub,p.done,p.total,(o.deadline!=null?daysUntil(o.deadline):null),goto1(o));}).join("");}
    if(otherO.length){h+='<div class="goals-tag">\ud83c\udfaf Autres objectifs</div>'+otherO.map(function(o){var badge=o.deadline?('<span class="goal-badge">'+dtxt(daysUntil(o.deadline))+'</span>'):"";return '<div class="goal"><div class="goal-head"><div class="goal-name">'+esc(o.icon)+" "+esc(o.name)+'</div>'+badge+'</div><div class="goal-sub">\ud83d\udea7 Suivi \u00e0 venir'+(o.deadline?(' \u00b7 '+esc(frDateShort(o.deadline))):"")+'</div></div>';}).join("");}
    h+='</div>';
    host.innerHTML=h;
  ...
  function wbOrder(d){
    var pool=wbPool(d).filter(function(it){return !it.done;});
  ...
  function renderGesteMoment(){
    ...
    var iso=todayStr(),order=wbOrder(iso);
    if(!order.length){
      var setup=(typeof setupRows==="function")?setupRows():[];
      if(setup.length){
        host.innerHTML='<div class="card pad gm-card"><div class="gm-h">\ud83d\udccb \u00c0 renseigner une fois</div>'+
          '<div class="gm-setup">'+setup.map(function(r,i){return '<button type="button" class="gm-stp" data-i="'+i+'"><span class="gm-stp-ic">'+r.ic+'</span><span class="gm-stp-tx"><b>'+esc(r.lbl)+'</b><span class="gm-stp-why">'+esc(r.why)+'</span></span><span class="gm-stp-go">\u203a</span></button>';}).join("")+'</div>'+
          '<div class="gm-foot">Ton bien-\u00eatre du jour est fait \u2014 il reste ces r\u00e9glages ponctuels. Une fois saisis, ils disparaissent.</div></div>';
        host.querySelectorAll(".gm-stp").forEach(function(b){b.onclick=function(){var r=setup[parseInt(b.getAttribute("data-i"),10)];if(r&&typeof openSettingsAt==="function")openSettingsAt(r.grp,r.sec);};});
        return;
      }
      host.innerHTML='<div class="card pad gm-card gm-done"><div class="gm-h">🌿 Bien-être du jour</div><div class="gm-alldone">✓ Tout ton bien-être du jour est coché. Beau boulot.</div></div>';
      return;
    }
  ...
  function weekVizHTML(){
    ...
    return '<div class="card pad"><div class="sec-head"><div class="sec-title">7 derniers jours</div>'+
      '<details class="base-hint inl sec-i"><summary>i</summary><div>Barre pleine couleur = cible atteinte · gris clair = pas de donnée.</div></details></div>'+
  ...
  function regGridHTML(rows,days){
    ...
    rows.forEach(function(r){
      h+='<div class="act-row"><span class="act-label'+(r.jump?" go":"")+'"'+(r.jump?' data-jump="'+r.jump+'"':"")+' title="'+esc(r.name)+'" aria-label="'+esc(r.name)+'">'+esc(r.ic||"\u2022")+'</span>'+
  ...
  function renderRegularity(){
    ...
    var sportRows=tag(["Muscu","Course","V\u00e9lo","Natation","Escalade"].map(function(sp){
      return {ic:SPORT_IC[sp],name:sp,s:0,jump:"sport",on:function(d){var l=(state.days[d]&&state.days[d].sports)||[];return l.indexOf(sp)>-1;}};
    }));
    var routRows=[];
    (typeof ROUTINES!=="undefined"?ROUTINES:[]).forEach(function(r){routRows.push({ic:r.icon,name:r.name,jump:"rx",s:currentStreak(function(iso){return routineDoneOn(r.id,iso);}),on:function(d){return routineDoneOn(r.id,d);}});});
    customRoutines().forEach(function(a){routRows.push({ic:a.icon,name:a.label,jump:"cr",s:currentStreak(function(iso){return crDoneOn(a.id,iso);}),on:function(d){return crDoneOn(a.id,d);}});});
    tag(routRows);
    var pxRows=tag((typeof PETITS_EXOS!=="undefined"?PETITS_EXOS:[]).map(function(r){
      return {ic:r.icon,name:r.name,jump:"px",s:currentStreak(function(iso){return pxDoneOn(r.id,iso);}),on:function(d){return pxDoneOn(r.id,d);}};
    }));
    var px=pxStats(30);
    function sub(k,ttl,n,inner){return '<div class="reg-sub">'+bndHead(k,"sub",{ttl:ttl,meta:n+"/"+REG_N+" j"})+bndBody(k,"",inner)+'</div>';}

    host.innerHTML='<div class="card pad"><div class="sec-head"><div class="sec-title">R\u00e9gularit\u00e9</div>'+
      '<details class="base-hint inl sec-i"><summary>i</summary><div>La s\u00e9rie court tant qu\'une habitude, un ancrage ou un petit exercice est coch\u00e9 chaque jour. Ci-dessous les 14 derniers jours : case pleine = fait, contour color\u00e9 = aujourd\'hui. Touche un emoji pour lire son nom ; \u00e0 droite, \ud83d\udd25 = s\u00e9rie en cours, \u00d7 = nombre de jours sur 14.</div></details></div>'+
    ...
    '</div>';
    host.querySelectorAll(".act-label[data-jump]").forEach(function(b){b.onclick=function(){jumpDay(b.getAttribute("data-jump"));};});
  }
  ...
  /* Reglages a saisir une fois : une ligne chacun, un acces direct au champ,
     et la ligne disparait des que la valeur est la. C'est une liste de courses
     qui doit s'epuiser — a l'oppose du jeu quotidien des pastilles. */
  function setupRows(){
    var pf=profileGet(),th=thresholdsGet(),n7=weightLogs7(),rows=[];
    function empty(v){return v===undefined||v===null||(""+v).trim()==="";}
    function add(ic,ok,lbl,why,grp,sec){if(!ok)rows.push({ic:ic,lbl:lbl,why:why,grp:grp,sec:sec});}
    add("\ud83d\udccf",!empty(pf.height),"Taille","indispensable pour estimer ta d\u00e9pense \u00e9nerg\u00e9tique.","g_profil","profil");
    add("\ud83c\udf82",!empty(pf.age),"\u00c2ge","affine le calcul de ton m\u00e9tabolisme de base.","g_profil","profil");
    add("\ud83e\uddcd",n7>=2,"Pes\u00e9e",n7+"/2 pes\u00e9es sur 7 jours \u2014 deux suffisent \u00e0 lisser le bruit et voir la vraie tendance.","g_profil","profil");
    add("\ud83d\udc5f",!empty(th.vma),"VMA","d\u00e9bloque tes allures de course et ta VO2max.","g_profil","seuils");
    add("\ud83d\udeb4",!empty(th.ftp),"FTP v\u00e9lo","d\u00e9bloque tes zones de puissance.","g_profil","seuils");
    add("\u2764\ufe0f",!empty(th.fcmax),"FC max","d\u00e9bloque tes zones cardio.","g_profil","seuils");
    return rows;
  }
  function renderSetupList(){
    var host=document.getElementById("setupList");if(!host)return;
    var rows=setupRows();
    if(!rows.length){host.innerHTML="";return;}
    host.innerHTML='<details class="card pad stp-card stp-fold"><summary class="stp-sum"><span class="stp-sum-l">\ud83d\udccb <b>\u00c0 renseigner une fois</b></span><span class="stp-sum-n">'+rows.length+'</span></summary>'+
      '<p class="muted stp-intro">Chaque ligne dispara\u00eet d\u00e8s que la valeur est saisie.</p>'+
      rows.map(function(r,i){
        return '<button type="button" class="stp-row" data-i="'+i+'">'+
          '<span class="stp-ic">'+r.ic+'</span>'+
          '<span class="stp-txt"><b>'+esc(r.lbl)+'</b><span class="stp-why">'+esc(r.why)+'</span></span>'+
          '<span class="stp-go">\u203a</span></button>';
      }).join("")+'</details>';
    host.querySelectorAll(".stp-row").forEach(function(b){
      b.onclick=function(){var r=rows[parseInt(b.getAttribute("data-i"),10)];if(r)openSettingsAt(r.grp,r.sec);};
    });
  }
  ...
  function actName(k){if(k==="tri"){var _tl=triLink();if(_tl&&_tl.name)return _tl.name;}if(k==="muscu"){var _ml=muscuLink();if(_ml&&_ml.name)return _ml.name;}return cfgActs()[k].name||(k==="muscu"?"Musculation":"Triathlon");}
  function setAct(k,patch){var a=cfgActs()[k];for(var p in patch)if(patch.hasOwnProperty(p))a[p]=patch[p];save();}
  function triRaceDate(){var a=cfgActs().tri;return (a&&a.raceDate)||RACE_DATE;}
  function triTargets(){var a=cfgActs().tri,d=(a&&a.dist)||{},out={};for(var k in TRI_TARGETS){var v=(d[k]!=null&&d[k]!=="")?num(d[k]):TRI_TARGETS[k].v;out[k]={v:(isNaN(v)?TRI_TARGETS[k].v:v),u:TRI_TARGETS[k].u,icon:TRI_TARGETS[k].icon};}return out;}
  /* Config utilisateur des sports de la bibliothèque (activation + niveau actuel/visé). state.config.sportsUser — pas de nouvelle clé. */
  function sportsUser(){if(!state.config)state.config={};if(!state.config.sportsUser)state.config.sportsUser={};return state.config.sportsUser;}
  function sportUCfg(id){var u=sportsUser(),sp=(typeof SPORTS_LIB!=="undefined"&&SPORTS_LIB[id])||{};if(!u[id])u[id]={on:!!sp.defaultOn,cur:0,target:0,minutes:45,days:[],lvlBase:0,deadline:""};if(u[id].minutes==null)u[id].minutes=45;if(!u[id].days)u[id].days=[];if(u[id].lvlBase==null)u[id].lvlBase=0;if(u[id].deadline==null)u[id].deadline="";return u[id];}
  function setSportU(id,patch){var c=sportUCfg(id);for(var k in patch)if(patch.hasOwnProperty(k))c[k]=patch[k];sportPlanCache=null;save();}
  /* Prévision : ~8 séances par palier. Selon jours/sem + deadline → « ça rentre ? » et fréquence nécessaire. */
  var SESS_PER_LEVEL=8;
  function sportForecast(id){
    var uc=sportUCfg(id),cur=num(uc.cur)||0,tgt=num(uc.target)||0;
    if(tgt<=cur)return null;
    var need=(tgt-cur)*SESS_PER_LEVEL,perWeek=(uc.days||[]).length,r={need:need,perWeek:perWeek,steps:tgt-cur,deadline:uc.deadline||""};
    if(perWeek>0){r.weeks=Math.ceil(need/perWeek);r.finish=isoOf(addDays(todayStr(),r.weeks*7));}
    if(uc.deadline){var dl=daysUntil(uc.deadline);r.dlDays=dl;var wl=dl/7;r.neededPerWeek=(wl>0)?Math.ceil(need/wl):null;if(perWeek>0){r.fits=(r.weeks*7<=dl);r.marginW=Math.round((dl-r.weeks*7)/7);}}
    return r;
  }
  function forecastHTML(id){
    var f=sportForecast(id);if(!f)return"";
    var lines=[];
    lines.push('<b>~'+f.need+' séances</b> estimées pour '+f.steps+' palier'+(f.steps>1?'s':'')+' (~'+SESS_PER_LEVEL+'/palier).');
    if(f.perWeek>0&&f.finish){lines.push('À '+f.perWeek+' séance'+(f.perWeek>1?'s':'')+'/sem → objectif vers <b>'+esc(frDateShort(f.finish))+'</b> (~'+f.weeks+' sem).');}
    else if(!f.perWeek){lines.push('Choisis tes jours d\'entraînement pour estimer la date d\'atteinte.');}
    var verdict="";
    if(f.deadline){
      if(f.perWeek>0&&f.fits===true)verdict='<span class="fc-ok">✅ Ça rentre — marge ~'+Math.max(0,f.marginW)+' sem avant l\'échéance.</span>';
      else if(f.perWeek>0&&f.fits===false)verdict='<span class="fc-no">⚠️ Trop juste : fini ~'+Math.abs(f.marginW)+' sem après l\'échéance. Il faudrait ~'+f.neededPerWeek+' séances/sem.</span>';
      else if(f.neededPerWeek)verdict='<span class="fc-no">Pour tenir l\'échéance : ~<b>'+f.neededPerWeek+' séances/sem</b>.</span>';
    }
    return '<div class="splib-fc">'+lines.map(function(l){return '<div>'+l+'</div>';}).join("")+(verdict?'<div class="fc-v">'+verdict+'</div>':"")+'</div>';
  }
  function sportSessDone(id,iso){var m=state.sportSess||{};return !!m[id+"|"+iso];}
  function sportDoneCount(id){var m=state.sportSess||{},c=0,pre=id+"|";for(var k in m)if(m.hasOwnProperty(k)&&k.indexOf(pre)===0&&m[k])c++;return c;}
  function toggleSportSess(id,iso){if(!state.sportSess)state.sportSess={};var k=id+"|"+iso;if(state.sportSess[k])delete state.sportSess[k];else state.sportSess[k]=true;sportPlanCache=null;save();}
  ...
  function muscuSeqAll(){if(!actEnabled("muscu"))return [];var s=[];BLOCK_ORDER.forEach(function(b){var wks=PROGRAM_BLOCKS[b].weeks,cd=codesOf(b);for(var w=1;w<=wks;w++){cd.forEach(function(c){s.push({kind:"muscu",block:b,w:w,code:c});});}});return s;}
  ...
  /* Séances de sport (bibliothèque) : couche SÉPARÉE, superposée au calendrier — ne touche pas buildSchedule.
     Sur les jours choisis (uc.days), pour un sport activé avec un objectif en cours, à partir d'aujourd'hui. */
  function sportPlan(){if(!state.config)state.config={};if(!state.config.sportPlan)state.config.sportPlan={perDay:1,maxConsec:3};var p=state.config.sportPlan;if(p.perDay==null)p.perDay=1;if(p.maxConsec==null)p.maxConsec=3;return p;}
  function setSportPlan(patch){var p=sportPlan();for(var k in patch)if(patch.hasOwnProperty(k))p[k]=patch[k];sportPlanCache=null;save();}
  function sportRemaining(id){var uc=sportUCfg(id);return Math.max(0,(num(uc.target)-num(uc.cur))*SESS_PER_LEVEL - sportDoneCount(id));}
  var sportPlanCache=null;
  function rebuildSportPlan(){
    var map={};sportPlanCache=map;
    if(typeof SPORTS_LIB==="undefined"||!SPORTS_LIB)return;
    var excl={muscu:1,nage:1,velo:1,course:1},sports=[];
    (SPORTS_LIB.order||[]).forEach(function(id){if(excl[id])return;var uc=sportUCfg(id);if(uc.on===false)return;if(!(num(uc.target)>num(uc.cur)))return;var days=(uc.days||[]);if(!days.length)return;var rem=sportRemaining(id);if(rem>0)sports.push({id:id,rem:rem,days:days});});
    if(!sports.length)return;
    var P=sportPlan(),perDay=Math.max(1,num(P.perDay)||1),maxConsec=Math.max(1,num(P.maxConsec)||3),cur=todayStr(),consec=0,guard=0;
    function anyRem(){return sports.some(function(s){return s.rem>0;});}
    while(anyRem()&&guard<400){
      guard++;
      if(consec>=maxConsec){consec=0;cur=isoOf(addDays(cur,1));continue;}
      var dow=new Date(cur+"T00:00:00").getDay(),placed=[],cnt=0;
      for(var i=0;i<sports.length&&cnt<perDay;i++){var s=sports[i];if(s.rem>0&&s.days.indexOf(dow)>=0){placed.push(s.id);s.rem--;cnt++;}}
      if(placed.length){map[cur]=placed;consec++;}else{consec=0;}
      cur=isoOf(addDays(cur,1));
    }
  }
  function sportSessionsOn(iso){
    if(typeof SPORTS_LIB==="undefined"||!SPORTS_LIB)return [];
    if(sportPlanCache===null)rebuildSportPlan();
    var out=[],seen={},excl={muscu:1,nage:1,velo:1,course:1};
    (SPORTS_LIB.order||[]).forEach(function(id){if(excl[id])return;if(sportSessDone(id,iso)){var sp=SPORTS_LIB[id]||{};out.push({id:id,icon:sp.icon||"•",name:sp.name||id,done:true});seen[id]=1;}});
    (sportPlanCache[iso]||[]).forEach(function(id){if(seen[id])return;var sp=SPORTS_LIB[id]||{};out.push({id:id,icon:sp.icon||"•",name:sp.name||id,done:false});});
    return out;
  }
  ...
  function legendHTML(){
    return '<details class="base-hint inl cal-i"><summary>i</summary><div class="cal-leg"><span>🏋️ '+esc(actName("muscu"))+'</span><span>🏊🚴🏃 '+esc(actName("tri"))+'</span><span>🧗🎾 Autres sports</span><span>📚 Révision</span><span><i class="lg ev-dot"></i>Événement</span><span class="lg-note">Séance faite = grisée + ✓ · touche un jour pour changer son état</span></div></details>';
  }
  function monthGridHTML(y,m){
    ...
    while(cur<=lastEnd&&guard<8){
      ...
      for(var i=0;i<7;i++){
        ...
        var pills=sessionsOn(iso).map(function(p){var ic=(p.kind==="muscu")?"🏋️":triIcon(p.disc);var ab=(p.kind==="muscu")?p.abbr:"";return '<span class="pill '+(p.kind==="muscu"?"p-muscu":"p-tri")+(p.done?" done":"")+'"><span class="pill-ic">'+ic+'</span>'+(ab?'<span class="pill-ab">'+esc(ab)+'</span>':'')+(p.done?'<span class="pill-ck">✓</span>':'')+'</span>';}).join("")+sportSessionsOn(iso).map(function(p){return '<span class="pill p-sport'+(p.done?" done":"")+'" title="'+esc(p.name)+'"><span class="pill-ic">'+esc(p.icon)+'</span>'+(p.done?'<span class="pill-ck">✓</span>':'')+'</span>';}).join("");
        var book="";
  ...
  function renderCalendars(){
    rebuildSchedule();rebuildSportPlan();
  ...
  function openDaySheet(iso){
    ...
    var spl=sportSessionsOn(iso);
    var spHTML=spl.length?('<div class="sheet-sports">'+spl.map(function(p){return '<button type="button" class="sheet-sp'+(p.done?" done":"")+'" data-spid="'+esc(p.id)+'"><span class="sheet-sp-n">'+esc(p.icon)+' '+esc(p.name)+'</span><span class="sheet-sp-ck">'+(p.done?"✓ fait":"à faire")+'</span></button>';}).join("")+'</div>'):"";
    ...
    sheet.querySelectorAll(".sheet-sp").forEach(function(b){b.addEventListener("click",function(){toggleSportSess(b.getAttribute("data-spid"),iso);renderCalendars();renderDayRadar();openDaySheet(iso);});});
  ...
  function actForm(k){var a=cfgActs()[k];return '<div class="act-form">'+
    ...
    (k==="tri"?(
      '<label class="act-flabel">Date de la course</label><input type="date" class="act-f-race" value="'+esc(triRaceDate())+'">'+
      '<label class="act-flabel">Distances cibles</label>'+
      '<div class="act-dist">'+
        '<label>🏊 Nat. (m)<input type="number" inputmode="numeric" step="1" class="act-f-dnat" value="'+esc(triTargets().nat.v)+'"></label>'+
        '<label>🚴 Vélo (km)<input type="number" inputmode="decimal" step="0.1" class="act-f-dvelo" value="'+esc(triTargets().velo.v)+'"></label>'+
        '<label>🏃 Course (km)<input type="number" inputmode="decimal" step="0.1" class="act-f-dcourse" value="'+esc(triTargets().course.v)+'"></label>'+
      '</div>'
    ):(
      '<label class="act-flabel">Objectif — échéance (optionnel)</label><input type="date" class="act-f-deadline" value="'+esc(a.deadline||"")+'">'
    ))+
  ...
  function wireActForm(){
    ...
    f.querySelector(".act-f-save").onclick=function(){
      ...
      var patch={name:name,start:start,desc:desc};
      if(k==="tri"){
        var rd=(f.querySelector(".act-f-race")||{}).value;if(rd&&(""+rd).trim())patch.raceDate=(""+rd).trim();
        var dn=(f.querySelector(".act-f-dnat")||{}).value,dv=(f.querySelector(".act-f-dvelo")||{}).value,dc=(f.querySelector(".act-f-dcourse")||{}).value;
        patch.dist={nat:(""+(dn||"")).trim(),velo:(""+(dv||"")).trim(),course:(""+(dc||"")).trim()};
      }else{
        var dl=(f.querySelector(".act-f-deadline")||{}).value;patch.deadline=(""+(dl||"")).trim();
      }
      setAct(k,patch);settingsActEdit=null;renderSettings();renderCalendars();
    };
  }
  ...
  function fuelWrapHTML(){var p=fuelPlanGet();
   ...
   function planBlkHTML(){
var P=sportPlan(),pd=num(P.perDay)||1,mc=num(P.maxConsec)||3;
        return '<div class="splan-box"><div class="splan-h">🗓️ Planning des séances (calendrier)</div>'+
          '<div class="splan-row"><label>Séances / jour max<select class="splan-pd">'+[1,2,3].map(function(v){return '<option value="'+v+'"'+(v===pd?" selected":"")+'>'+v+'</option>';}).join("")+'</select></label>'+
          '<label>Jours d\'affilée max<select class="splan-mc">'+[2,3,4,5,6,7].map(function(v){return '<option value="'+v+'"'+(v===mc?" selected":"")+'>'+v+'</option>';}).join("")+'</select></label></div>'+
  ...
  function objBlkHTML(part){var _sl=(typeof SPORTS_LIB!=="undefined"&&SPORTS_LIB)?SPORTS_LIB:null;if(!_sl)return "";
        ...
        var list=objs.map(function(o){var spTags=(o.sports||[]).length?(o.sports||[]).map(function(sid){return '<span class="objc-sptag">'+esc((_sl[sid]||{}).icon||"")+' '+esc((_sl[sid]||{}).name||sid)+'</span>';}).join(""):('<span class="objc-sptag">'+esc({intellectuel:"🧠 Intellectuel",artistique:"🎨 Artistique",autre:"📌 Autre"}[o.kind]||"")+'</span>');var dlTag=o.deadline?('<span class="objc-dltag">📅 '+esc(frDateShort(o.deadline))+'</span>'):"";
          var isTri=(o.sports||[]).filter(function(s){return s==="nage"||s==="velo"||s==="course";}).length>=2;
          var T=isTri?triTargets():null;
          var triEdit=isTri?('<div class="objc-triedit"><div class="objc-te-h">🏁 Épreuve — pilote le programme triathlon</div><label class="objc-te-f">Date de course<input type="date" class="objc-race" value="'+esc(triRaceDate())+'"></label><div class="objc-te-dist"><label>🏊 m<input type="number" inputmode="numeric" class="objc-dnat" value="'+esc(T.nat.v)+'"></label><label>🚴 km<input type="number" inputmode="decimal" step="0.1" class="objc-dvelo" value="'+esc(T.velo.v)+'"></label><label>🏃 km<input type="number" inputmode="decimal" step="0.1" class="objc-dcourse" value="'+esc(T.course.v)+'"></label></div></div>'):"";
          var editing=(editObjId===o.id);
        ...
        var form=objCreating?('<div class="objc-form">'+
          '<label class="act-flabel">Nom de l\'objectif</label><input type="text" class="objc-name" placeholder="ex : Séjour escalade, DSCG octobre…">'+
          '<label class="act-flabel">Type</label><div class="objc-types"><button type="button" class="objc-type on" data-type="sport">🏅 Sport</button><button type="button" class="objc-type" data-type="intellectuel">🧠 Intellectuel</button><button type="button" class="objc-type" data-type="artistique">🎨 Artistique</button><button type="button" class="objc-type" data-type="autre">📌 Autre</button></div>'+
          '<div class="objc-sportsblk"><label class="act-flabel">Sports couverts &amp; niveaux</label><div class="objc-sports">'+(_sl.order||[]).map(function(sid){var sp=_sl[sid],uc=sportUCfg(sid),maxN=(sp.levels||[]).length-1,c=Math.max(0,Math.min(maxN,num(uc.cur)||0)),t=Math.max(0,Math.min(maxN,num(uc.target)||0));
  ...
  function renderSettings(){
    ...
    var _lnk={};cfgObjs().forEach(function(o){if(o.dlId)_lnk[o.dlId]=1;});
    var rows=pDeadlines().filter(function(d){return !_lnk[d.id];}).map(function(d){
      if(settingsEdit===d.id)return deadlineForm(d);
      return '<div class="set-row"><span class="set-ic">'+esc(d.icon||"🎯")+'</span><span class="set-main"><span class="set-lbl">'+esc(d.label)+'</span><span class="set-sub">'+esc(frDateShort(d.date))+' '+dlYear(d.date)+' · '+dlTxt(d.date)+'</span></span><button class="set-edit" data-edit="'+d.id+'" aria-label="Modifier">✎</button><button class="set-del" data-del="'+d.id+'" aria-label="Supprimer">🗑</button></div>';
    }).join("");
    ...
    var sportsLibInner=!_sl?'<p class="set-note">Bibliothèque indisponible.</p>':
      (_sl.order||[]).map(function(id){var sp=_sl[id];if(!sp)return "";
        ...
        function optsFor(selV){return (sp.levels||[]).map(function(lv){return '<option value="'+lv.n+'"'+(lv.n===selV?" selected":"")+'>'+lv.n+' · '+esc(lv.t)+'</option>';}).join("");}
        var body="";
        if(open){
          if(!on){
            body='<div class="splib-lad splib-dim">'+(sp.levels||[]).map(function(lv){
            ...
            if(tgt<=cur){prog='<div class="splib-hint">Choisis un niveau visé plus haut que ton niveau actuel pour afficher ton parcours.</div>';}
              ...
              (sp.levels||[]).filter(function(lv){return lv.n>cur&&lv.n<=tgt;}).map(function(lv){
                return '<div class="splib-step"><div class="splib-step-h"><span class="splib-n">'+lv.n+'</span><b>'+esc(lv.t)+'</b></div>'+
                  '<div class="splib-d">'+esc(lv.d)+'</div>'+
                  ((lv.focus&&lv.focus.length)?('<ul class="splib-focus">'+lv.focus.map(function(f){return '<li>'+esc(f)+'</li>';}).join("")+'</ul>'):"")+'</div>';
              }).join("")+'</div>';}
            var mins=num(uc.minutes)||45,days=(uc.days||[]).slice(),ORDER=[1,2,3,4,5,6,0],DNAME={1:"Lun",2:"Mar",3:"Mer",4:"Jeu",5:"Ven",6:"Sam",0:"Dim"};
            var cad='<div class="splib-cad"><span class="splib-cad-l">Jours d\'entraînement <span class="splib-cad-s">(= espacement des séances)</span></span>'+
              '<div class="splib-dows">'+ORDER.map(function(dw){var on2=days.indexOf(dw)>=0;return '<button type="button" class="splib-dow'+(on2?" on":"")+'" data-sp="'+esc(id)+'" data-dw="'+dw+'" aria-pressed="'+(on2?"true":"false")+'">'+DNAME[dw].charAt(0)+'</button>';}).join("")+'</div>'+
              '<label class="splib-durl">Durée / séance<select class="splib-min" data-sp="'+esc(id)+'">'+[30,45,60,75,90].map(function(v){return '<option value="'+v+'"'+(v===mins?" selected":"")+'>'+v+' min</option>';}).join("")+'</select></label></div>';
            var week="";
            if(tgt>cur){
              var nextLv=(sp.levels||[])[cur+1]||{},themes=(nextLv.focus&&nextLv.focus.length)?nextLv.focus:["Séance libre sur ce sport"];
              if(!days.length){week='<div class="splib-hint">Choisis tes jours d\'entraînement ci-dessus : ta semaine type se génère et les séances apparaissent dans le calendrier.</div>';}
              else{var od=ORDER.filter(function(dw){return days.indexOf(dw)>=0;});
                week='<div class="splib-week"><div class="splib-prog-h">Semaine type — cap sur le palier '+(cur+1)+' · '+esc(nextLv.t||"")+'</div>'+
                  od.map(function(dw,i){var th=themes[i%themes.length];return '<div class="splib-sess"><span class="splib-sess-n">'+DNAME[dw]+'</span><span class="splib-sess-tx"><b>'+esc(th)+'</b><span class="splib-sess-min">~'+mins+' min</span></span></div>';}).join("")+
                  '<div class="splib-hint" style="margin-top:8px">Ces séances apparaissent dans le calendrier sur les jours choisis (à venir).</div></div>';}
            }
            body=sel+prog+cad+week;
            if(tgt>cur){var doneN=sportDoneCount(id),since=Math.max(0,doneN-(num(uc.lvlBase)||0)),THR=8,nx=(sp.levels||[])[cur+1]||{};
              body+=forecastHTML(id);
              body+=(since>=THR)?
                ('<div class="splib-levelup"><div class="splib-lu-h">🎉 '+since+' séances validées depuis le niveau '+cur+'.</div><div class="splib-lu-d">Prêt à passer au palier '+(cur+1)+' · '+esc(nx.t||"")+' ?</div><button type="button" class="btn accent splib-lvbtn" data-sp="'+esc(id)+'">Passer au niveau '+(cur+1)+'</button></div>')
                :('<div class="splib-prog2">Séances validées à ce palier : <b>'+since+'</b> / '+THR+' <span class="splib-prog2-s">avant de te suggérer le palier suivant</span></div>');
            }
          }
        }
        var badge=on?((tgt>cur)?('niv. '+cur+' → '+tgt):('niv. '+cur)):'désactivé';
        return '<div class="splib-sport'+(open?" open":"")+(on?"":" off")+'">'+
          '<button type="button" class="splib-head" data-splib="'+esc(id)+'"><span class="splib-ic">'+esc(sp.icon||"•")+'</span><span class="splib-name">'+esc(sp.name||id)+'</span><span class="splib-meta">'+badge+'</span><span class="bnd-chev">▾</span></button>'+
          (open?('<div class="splib-inner"><label class="splib-onoff"><input type="checkbox" class="splib-on" data-sp="'+esc(id)+'"'+(on?" checked":"")+'><span>Sport activé</span></label>'+body+'</div>'):"")+
        '</div>';
      }).join("");
    ...
    host.querySelectorAll(".set-sectog").forEach(function(b){b.onclick=function(){var id=b.getAttribute("data-sec");settingsSecOpen[id]=!settingsSecOpen[id];if(id==="objsport"&&!settingsSecOpen.objsport){settingsEdit=null;settingsActEdit=null;}renderSettings();};});
    host.querySelectorAll(".splib-head").forEach(function(b){b.onclick=function(){var id="splib_"+b.getAttribute("data-splib");settingsSecOpen[id]=!settingsSecOpen[id];renderSettings();};});
    host.querySelectorAll(".splib-on").forEach(function(cb){cb.onchange=function(){var sid=cb.getAttribute("data-sp");setSportU(sid,{on:cb.checked});if(sid==="muscu")setAct("muscu",{enabled:cb.checked});renderSettings();renderCalendars();};});
    host.querySelectorAll(".splib-cur").forEach(function(s){s.onchange=function(){var id=s.getAttribute("data-sp");setSportU(id,{cur:num(s.value),lvlBase:sportDoneCount(id)});renderSettings();};});
    host.querySelectorAll(".splib-lvbtn").forEach(function(b){b.onclick=function(){var id=b.getAttribute("data-sp"),uc=sportUCfg(id),mx=((SPORTS_LIB[id]||{}).levels||[]).length-1,nc=Math.min((num(uc.cur)||0)+1,mx);setSportU(id,{cur:nc,lvlBase:sportDoneCount(id)});renderSettings();};});
    host.querySelectorAll(".splib-tgt").forEach(function(s){s.onchange=function(){setSportU(s.getAttribute("data-sp"),{target:num(s.value)});renderSettings();};});
    host.querySelectorAll(".splib-dow").forEach(function(b){b.onclick=function(){var id=b.getAttribute("data-sp"),dw=num(b.getAttribute("data-dw")),uc=sportUCfg(id),d=(uc.days||[]).slice(),i=d.indexOf(dw);if(i>=0)d.splice(i,1);else d.push(dw);setSportU(id,{days:d});renderSettings();};});
    host.querySelectorAll(".splib-min").forEach(function(s){s.onchange=function(){setSportU(s.getAttribute("data-sp"),{minutes:num(s.value)});renderSettings();};});
    host.querySelectorAll(".splib-dl").forEach(function(s){s.onchange=function(){setSportU(s.getAttribute("data-sp"),{deadline:(s.value||"").trim()});renderSettings();};});
    ...
    host.querySelectorAll(".objc-type").forEach(function(b){b.onclick=function(){
      host.querySelectorAll(".objc-type").forEach(function(x){x.classList.remove("on");});b.classList.add("on");
      var isSport=b.getAttribute("data-type")==="sport";
      var sb=host.querySelector(".objc-sportsblk"),ib=host.querySelector(".objc-iconblk");
      if(sb)sb.hidden=!isSport;if(ib)ib.hidden=isSport;
    };});
    host.querySelectorAll(".objc-icon").forEach(function(b){b.onclick=function(){host.querySelectorAll(".objc-icon").forEach(function(x){x.classList.remove("on");});b.classList.add("on");};});
    host.querySelectorAll(".objc-sp").forEach(function(cb){cb.onchange=function(){var row=cb.closest(".objc-sprow"),lv=row?row.querySelector(".objc-nlv"):null;if(lv)lv.hidden=!cb.checked;};});
    var objCreate=host.querySelector(".objc-create");if(objCreate)objCreate.onclick=function(){
      var nm=((host.querySelector(".objc-name")||{}).value||"").trim();
      var tb=host.querySelector(".objc-type.on"),kind=tb?tb.getAttribute("data-type"):"sport";
      var dl=((host.querySelector(".objc-dl")||{}).value||"").trim();
      if(!nm){fqToast("Donne un nom à l'objectif");return;}
      var sps=[],icon="🎯";
      if(kind==="sport"){var checked=[].slice.call(host.querySelectorAll(".objc-sp:checked"));if(!checked.length){fqToast("Choisis au moins un sport");return;}
        checked.forEach(function(c){var sid=c.value,row=c.closest(".objc-sprow"),cu=row?num((row.querySelector(".objc-ncur")||{}).value):NaN,tg=row?num((row.querySelector(".objc-ntgt")||{}).value):NaN,patch={on:true,deadline:dl};if(!isNaN(cu))patch.cur=cu;if(!isNaN(tg))patch.target=tg;setSportU(sid,patch);sps.push(sid);});}
      else{var ib=host.querySelector(".objc-icon.on");icon=ib?ib.textContent:"📌";}
      var oid="obj-"+Date.now(),dlId=dl?pAddDeadline({date:dl,label:nm,icon:(kind==="sport"?"🏅":icon)}):null;
      addObj({id:oid,name:nm,icon:icon,sports:sps,deadline:dl,dlId:dlId,kind:kind});syncTri();objCreating=false;renderSettings();renderCalendars();refreshSportViews();
    };
    host.querySelectorAll(".objc-exp").forEach(function(b){b.onclick=function(){var id=b.getAttribute("data-obj");objExpand[id]=!objExpand[id];renderSettings();};});
    host.querySelectorAll(".objd-cur").forEach(function(s){s.onchange=function(){setSportU(s.getAttribute("data-sp"),{cur:num(s.value),lvlBase:sportDoneCount(s.getAttribute("data-sp"))});renderSettings();renderCalendars();refreshSportViews();};});
    host.querySelectorAll(".objd-tgt").forEach(function(s){s.onchange=function(){setSportU(s.getAttribute("data-sp"),{target:num(s.value)});renderSettings();renderCalendars();refreshSportViews();};});
    ...
    var oRace=host.querySelector(".objc-race");if(oRace)oRace.onchange=function(){setAct("tri",{enabled:true,raceDate:(oRace.value||"").trim()});renderSettings();renderCalendars();};
    (function(){var dn=host.querySelector(".objc-dnat"),dv=host.querySelector(".objc-dvelo"),dc=host.querySelector(".objc-dcourse");function sd(){setAct("tri",{enabled:true,dist:{nat:(dn?dn.value:""),velo:(dv?dv.value:""),course:(dc?dc.value:"")}});renderCalendars();}if(dn)dn.onchange=sd;if(dv)dv.onchange=sd;if(dc)dc.onchange=sd;})();
    ...
    (function(){var pf=profileGet();
      function pfBmrRefresh(){var el=host.querySelector(".pf-bmr");if(!el)return;var b=bmr(),w=lastWeight();el.innerHTML=(b!=null?('Métabolisme de base : <b>'+Math.round(b)+' kcal/j</b>'+(w!=null?(' · poids '+fr1(w)+' kg'):'')):'<span class="low">Complète taille + âge pour le calcul.</span>');}
      var sx=host.querySelector(".pf-sex");if(sx)sx.onchange=function(){pf.sex=sx.value;save();pfBmrRefresh();};
      var h=host.querySelector(".pf-h");if(h)h.oninput=function(){pf.height=h.value;save();pfBmrRefresh();};
      var a=host.querySelector(".pf-age");if(a)a.oninput=function(){pf.age=a.value;save();pfBmrRefresh();};
      var w=host.querySelector(".pf-w");if(w)w.oninput=function(){pf.weight=w.value;save();pfBmrRefresh();};
    ...
    host.querySelectorAll(".ff-reset").forEach(function(b){b.onclick=function(){var k=b.getAttribute("data-k");if(state.foodFix)delete state.foodFix[k];save();settingsFoodSel=null;renderSettings();refreshDayForm();};});
    host.querySelectorAll(".ff-migrate").forEach(function(b){b.onclick=function(){var k=b.getAttribute("data-k"),cnt=b.getAttribute("data-n");
      ...
      var done=migrateFood(k);save();settingsFoodSel=null;renderSettings();refreshDayForm();
    ...
    host.querySelectorAll(".fqe-save").forEach(function(b){b.onclick=function(){var k=b.getAttribute("data-k");var box=b.closest(".fqe-edit");if(!box)return;
      ...
      foodQualMap()[k]={p:p,n:n,w:w};fqInvalidate();save();settingsQualSel=null;renderSettings();refreshDayForm();};});
    host.querySelectorAll(".fqe-reset").forEach(function(b){b.onclick=function(){var k=b.getAttribute("data-k");if(state.foodQual)delete state.foodQual[k];fqInvalidate();save();settingsQualSel=null;renderSettings();refreshDayForm();};});
...
/* Migration ponctuelle : le plan tri est renuméroté en 4 semaines (départ 2026-08-17).
  ...
  function init(){
    ...
    var cc=document.getElementById("calChip");if(cc)cc.addEventListener("click",function(){
    ...
    wireBnd();wireAxis();wireStick();syncStickTop();wireFqTaps();loadFoodDB();
    ...
    var dlb=document.getElementById("dayLabel");if(dlb)dlb.addEventListener("click",function(){if(dayDate!==todayStr()){dayDate=todayStr();renderDay();window.scrollTo(0,0);}});
    ...
    migrateWeekKeys();pRenameDeadlinesOnce();pEnsureSeed();pMigrateStates();pMigrateDayTypes();seedPlanOnce();triStartMigrateOnce();triReslotByDate();migrateTriLink();migrateMuscuLink();syncTri();migrateOrphanSportsOnce();migrateObjLinks();
