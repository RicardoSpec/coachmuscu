...
function fqTokens(s){var STOP={de:1,du:1,des:1,au:1,aux:1,a:1,la:1,le:1,les:1,l:1,et:1,en:1,d:1,un:1,une:1};s=fqKey(s).replace(/[^a-z0-9]+/g," ");return s.split(" ").filter(function(t){return t.length>=2&&!STOP[t]&&!/^\d+$/.test(t);});}
  ...
  function syncStickTop(){
    ...
    /* La bande de pastilles se fige juste sous la date ; les bandeaux figés se
       posent sous elle. Un seul empilement, jamais deux niveaux concurrents. */
    var od=document.querySelector("#v-day .onb-dots");
    var oh=od?Math.round(od.getBoundingClientRect().height):0;
    r.style.setProperty("--bnd-top3",(base+(oh>0?oh+6:0))+"px");
  }
  function wireStick(){
    var tick=false;
    function upd(){
  ...
  function onbChecks(){
    ...
    /* (a) LES AXES DU RADAR — toujours affichés, même au vert. C'est tout
       l'intérêt : voir la pastille basculer de rouge à vert dans la journée. */
    var AXJ=["sport","prot","sleep","water","cr","bal"];
    var AXW=["bouger un peu chaque jour entretient la r\u00e9cup\u00e9ration autant que la forme.",
             "c'est la brique de la construction musculaire \u2014 mieux vaut l'\u00e9taler sur la journ\u00e9e.",
             "c'est la nuit que le muscle se r\u00e9pare, pas \u00e0 la salle.",
             "quelques pour cent de d\u00e9shydratation suffisent \u00e0 faire chuter la performance.",
             "une seule habitude tenue suffit \u00e0 prolonger la s\u00e9rie \ud83d\udd25.",
             "pour la pr\u00e9pa triathlon, l'enjeu est d'alimenter l'effort, pas de creuser un d\u00e9ficit."];
    radarDay(d).forEach(function(a,i){
      arr.push({ic:a.ic,ok:a.met,lbl:a.lab,jump:AXJ[i],
        txt:"<b>"+esc(a.lab)+"</b> \u00b7 "+esc(a.valTxt)+" \u2014 "+AXW[i]});
    });
    /* (b) HABITUDES DU JOUR — même logique de pastille, elles repassent au
       rouge chaque matin. */
    var nS=(typeof SUPPS!=="undefined"?SUPPS:[]).filter(function(sp){return x.supps&&x.supps[sp.id];}).length+((x.suppsX||[]).length);
    arr.push({ic:"\ud83d\udc8a",ok:nS>=2,lbl:"Compl\u00e9ments",jump:"supps",
      txt:"<b>"+nS+"/2 compl\u00e9ments</b> logg\u00e9s aujourd\u2019hui \u2014 c'est la r\u00e9gularit\u00e9 qui les rend utiles, pas la dose."});
    ...
    arr.push({ic:"\ud83c\udf3f",ok:nR>=2,lbl:"Bien-\u00eatre",jump:"rx",rx:!!sug,
      txt:"<b>"+nR+"/2</b> choses qui te font du bien aujourd\u2019hui"+(sug?(" \u2014 essaie <b>"+(sug.icon?esc(sug.icon)+" ":"")+esc(sug.name)+"</b>."):".")});
    /* (c) RÉGLAGES À FAIRE UNE FOIS — poussés seulement tant qu'ils manquent :
       une pastille verte figée pour toujours n'apprend rien. */
    function setup(ic,ok,lbl,txt,grp,sec){if(!ok)arr.push({ic:ic,ok:false,lbl:lbl,txt:txt,grp:grp,sec:sec});}
    setup("\ud83d\udccf",!empty(pf.height),"Taille","Ta <b>taille</b> n\u2019est pas renseign\u00e9e \u2014 indispensable pour estimer ta d\u00e9pense \u00e9nerg\u00e9tique.","g_profil","profil");
    setup("\ud83e\uddcd",weightLogs7()>=2,"Pes\u00e9e","<b>"+weightLogs7()+"/2 pes\u00e9es</b> sur les 7 derniers jours \u2014 deux par semaine suffisent \u00e0 lisser le bruit et voir la vraie tendance.","g_profil","profil");
    setup("\ud83c\udf82",!empty(pf.age),"\u00c2ge","Ton <b>\u00e2ge</b> n\u2019est pas renseign\u00e9 \u2014 il affine le calcul de ton m\u00e9tabolisme de base.","g_profil","profil");
    setup("\ud83d\udc5f",!empty(th.vma),"VMA","Ta <b>VMA</b> n\u2019est pas renseign\u00e9e \u2014 elle d\u00e9bloque tes allures de course et ta VO2max.","g_profil","seuils");
    setup("\ud83d\udeb4",!empty(th.ftp),"FTP","Ton <b>FTP v\u00e9lo</b> n\u2019est pas renseign\u00e9 \u2014 il d\u00e9bloque tes zones de puissance.","g_profil","seuils");
    setup("\u2764\ufe0f",!empty(th.fcmax),"FC max","Ta <b>FC max</b> n\u2019est pas renseign\u00e9e \u2014 elle d\u00e9bloque tes zones cardio.","g_profil","seuils");
    return arr;
  }
  ...
  function renderOnboard(){
    ...
    /* Les alertes ne parlent que d'aujourd'hui : muettes sur un jour pass\u00e9. */
    if(onbHidden||dayDate!==todayStr()){host.innerHTML="";return;}
    var all=onbChecks(),miss=[];
    all.forEach(function(o,i){if(!o.ok)miss.push(i);});
    if(!all.length){host.innerHTML="";return;}
    if(onbMin){
      host.innerHTML='<button type="button" class="onb-pill" id="onbExpand">'+
        (miss.length?("\u26a0\ufe0f "+miss.length+" \u00e0 compl\u00e9ter"):"\u2705 tout est au vert")+'</button>';
      var ex=document.getElementById("onbExpand");if(ex)ex.onclick=function(){onbMin=false;renderOnboard();};
      return;
    }
    if(onbIdx>=all.length||all[onbIdx].ok)onbIdx=miss.length?miss[0]:0;
    function ctlHTML(){return '<div class="onb-ctl"><button type="button" class="onb-min" aria-label="R\u00e9duire">\u2013</button><button type="button" class="onb-x" aria-label="Fermer">\u00d7</button></div>';}
    function topHTML(){
      if(!miss.length)return '<div class="onb-top"><div class="onb-ic">\u2705</div>'+
        '<div class="onb-body"><div class="onb-msg">Tout est au vert aujourd\u2019hui. Rien \u00e0 rattraper \u2014 c\u2019est exactement l\u2019objectif.</div></div>'+ctlHTML()+'</div>';
      var m=all[onbIdx];
      var act=m.jump
      ...
      return '<div class="onb-top"><div class="onb-ic">'+m.ic+'</div>'+
        '<div class="onb-body"><div class="onb-msg">'+(m.ok?('<b>'+esc(m.lbl)+'</b> \u2713 c\u2019est bon pour aujourd\u2019hui.'):m.txt)+'</div>'+
          '<div class="onb-actions">'+(m.ok?'':act)+'</div></div>'+ctlHTML()+'</div>';
    }
    function dotsHTML(){return all.map(function(o,i){
      return '<button type="button" class="onb-dot'+(o.ok?' ok':' ko')+((i===onbIdx)?' cur':'')+'" data-i="'+i+'" aria-label="'+esc(o.lbl)+'">'+o.ic+'</button>';}).join("");}
    function wire(){
      var m=all[onbIdx];
      ...
      host.querySelectorAll(".onb-dot").forEach(function(b){b.onclick=function(){onbStep(0,parseInt(b.getAttribute("data-i"),10));};});
    }
    function refresh(){
      var c=host.querySelector(".onb-card"),dd=host.querySelector(".onb-dots");
      if(c)c.innerHTML=topHTML();
      if(dd)dd.innerHTML=dotsHTML();
      wire();
    }
    /* dir=-1/+1 pour naviguer, ou to=index direct. Toute action manuelle coupe
       la rotation automatique : on ne se fait pas voler l'\u00e9cran en lisant. */
    function onbStep(dir,to){
      if(to==null){
        var pool=miss.length?miss:all.map(function(_,i){return i;});
        var pos=pool.indexOf(onbIdx);if(pos<0)pos=0;
        onbIdx=pool[(pos+dir+pool.length)%pool.length];
      }else onbIdx=to;
      if(onbTimer){clearInterval(onbTimer);onbTimer=null;}
      refresh();
    }
    host.innerHTML='<div class="onb-card">'+topHTML()+'</div><div class="onb-dots">'+dotsHTML()+'</div>';
    wire();
    /* Glissement horizontal sur la carte = alerte suivante / pr\u00e9c\u00e9dente.
       stopPropagation, sinon wireSwipe change d'onglet sous le doigt. */
    (function(){
      var c=host.querySelector(".onb-card");if(!c)return;
      var sx=0,sy=0,ok2=false;
      c.addEventListener("touchstart",function(e){e.stopPropagation();
        if(!e.touches||e.touches.length!==1){ok2=false;return;}
        sx=e.touches[0].clientX;sy=e.touches[0].clientY;ok2=true;},{passive:true});
      c.addEventListener("touchend",function(e){e.stopPropagation();
        if(!ok2)return;ok2=false;
        var t=e.changedTouches&&e.changedTouches[0];if(!t)return;
        var dx=t.clientX-sx,dy=t.clientY-sy;
        if(Math.abs(dx)<45||Math.abs(dx)<Math.abs(dy)*1.5)return;
        onbStep(dx<0?1:-1);},{passive:true});
    })();
  ...
  function buildDayForm(container,d){
    ...
    (function(){
      /* Résumé d'en-tête tenu à jour sans toucher aux handlers existants — et,
         dans la foulée, le radar et la bande de pastilles. Sans ce pouls, une
         pastille ne passe au vert qu'au prochain rendu complet : la récompense
         arrive trop tard pour être une récompense. Débounce : on ne re-rend pas
         le radar à chaque frappe dans le champ Poids. */
      var pt=null;
      function pulse(){
        dgMeta();
        if(pt)clearTimeout(pt);
        pt=setTimeout(function(){pt=null;renderDayRadar();renderOnboard();},220);
      }
    ...
    function recalcTotals(){var t=dayTotals(d);
      var _td=todoCount(),_tdEl=container.querySelector(".meal-todo");
      if(_tdEl){_tdEl.hidden=!_td;_tdEl.textContent=_td?("\u26a0\ufe0f "+_td+" \u00e0 chiffrer"):"";
        _tdEl.title=_td?("Ces aliments n'ont pas de valeurs nutritionnelles : ils comptent pour 0, ton total est donc sous-estim\u00e9."):"";}
      var el=container.querySelector(".meal-total");if(el){if(t){el.textContent=Math.round(t.kcal)+" kcal · "+fr1(pEff(t))+" g prot.";el.className="meal-total on";}else{el.textContent="";el.className="meal-total";}}var jt=container.querySelector('[data-bnd="jprot"]'),jb=container.querySelector(".jprot-body");if(jt&&jb){if(t){jt.hidden=false;jb.innerHTML=protBreakHTML(t)+dayMealDistHTML(d);jb.querySelectorAll(".md-mode").forEach(function(b){b.onclick=function(){mealDistMode=b.getAttribute("data-mode");recalcTotals();};});}else{jt.hidden=true;jb.innerHTML="";}}}
    ...
    function todoCount(){var n=0;MEALS.forEach(function(m){((day(d).mealItems||{})[m.k]||[]).forEach(function(it){if(itemTodo(it))n++;});});return n;}
